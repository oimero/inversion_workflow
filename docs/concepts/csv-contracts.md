...
`preprocess_status`、`selected_for_input`、`las_path`、
...
所有表使用 schema `forward_observability_v2`。上游来源可从配置显式指定，也可自动发现最新合格产物；发布摘要通过 `input_contracts` 记录直接上游契约。
...
`synthoseis_lite_impedance_calibration_v2`。
...
| 关键字段 | 含义 |
|----------|------|
| `schema_version` | 固定 `synthoseis_lite_impedance_calibration_v2` |
...
生成阶段冻结一个可复现的二维声阻抗合成基准。master schema 为 `synthoseis_lite_v3`。
...
| 关键字段 | 含义 |
|----------|------|
| `schema` / `schema_version` | 固定 `synthoseis_lite_v3` |
...
评估阶段产出模型无关的基线报告卡。schema 为 `synthoseis_lite_report_v2`。
...
| `input_contracts.benchmark` | 合成基准的直接契约指纹 |
...
| `loss` | 损失配置：`lambda_ai` / `lambda_physics` / `lambda_real_delta` / `physics_loss_applied_sample_kinds` |
| `model_info` | 参数量、感受野（lateral/twt）、输入/输出通道数 |
| `checkpoints` | `primary` 选择以及 `best`/`final` 的路径、epoch 和 validation loss；checkpoint 内容计入模型契约指纹 |
| `best_validation_loss` | 最佳校验损失 |
| `real_delta` | 可选真实井来源、holdout、预计算与井样点审计；纯 synthetic run 为 null |
| `real_well_outputs` | 可选全部井 best/final 指标、分频指标、波形指标和采样 QC |
| `synthetic_gate_evidence` | 训练发布时冻结的 gate 证据；R0 必检字段，缺失则拒绝推理 |
| `contract_fingerprint_sha256` | 模型 run 的唯一契约指纹 |
...
Step 6 schema 固定为 `real_field_well_controls_v3`，只保存 canonical 井控事实。

### `well_control_manifest.csv`

一行一口候选井。核心字段为 `well_name/status/reason/source_run_type`、source summary/LAS/transform 路径、domain/unit/depth basis、井型、sampling mode、有效样点统计和井 NPZ 路径。失败井保留行但没有可消费 NPZ。Step 6 摘要只发布一个契约指纹，并在 `input_contracts` 中记录 Step 1 与直接井源 run 的契约指纹。
...
Step 7 run schema 为 `real_field_lfm_run_v3`，variant schema 为 `real_field_lfm_variant_v3`。
...
记录业务配置、请求的 variant/comparison、Step 6 直接上游契约、输出几何和产物清单，并发布 run 级单一契约指纹。只有所有请求均成功并发布 manifest 后，`status` 才能为 `ok`。

## 08 R0 · real_field_zero_shot.py

R0 产物供 R1 消费。schema 为 `real_field_zero_shot_summary_v2`。
...
| 关键字段 | 含义 |
|----------|------|
| `schema_version` | 固定 `real_field_forward_diagnostic_summary_v3` |
| `status` | `ok` |
...
| `input_contracts` | R0 与子波等直接上游契约指纹 |
| `contract_fingerprint_sha256` | R1 发布契约的唯一指纹 |

## 旁路 · GINN-v2 Real-Delta 可选产物

### `real_delta_well_samples.csv`

当普通 GINN-v2 实验配置 `train.real_delta` 时生成，schema 为 `real_delta_well_samples_v3`。每行一个 canonical 井轨迹样点；该文件必须针对一次显式 LFM variant 单独生成。
...
| `variant_id` / `lfm_contract_fingerprint_sha256` / `well_control_contract_fingerprint_sha256` | 禁止跨 variant 共用 label 的直接契约身份 |
...
前者逐 checkpoint × 井保存分频 full-AI/delta 指标；后者保存井旁正演指标、QC 状态和
best/final 图件路径。

### `training_history.csv` / `real_delta_sampling_qc.csv`

前者逐 epoch 保存 synthetic/physics/real-delta/total loss、validation loss、耗时和采样覆盖；
后者逐井保存监督角色、有效样点数和累计抽样次数。所有排除井必须
`selected_count == 0`。
