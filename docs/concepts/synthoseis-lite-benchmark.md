...
首个实现切片固定生成表中前五个解析几何家族，共 14 个 realization。每个 realization
...
`frequency_probe` 尚未包含在该实现切片中，仍由第 6 节的第一闸门联接单独实现，不得
用手工固定频率补齐。
...
2. 沿配置折线按 `lateral_sample_interval_m` 等物理距离重采样所有层位；折点处保持累计
   路径距离连续，层位值通过 `HorizonSurface.sample_at_line()` 的双线性规则获得。
...
校准 JSON 必须包含上述两个 CSV 的 SHA-256。`generate` 不得修改这些文件。
