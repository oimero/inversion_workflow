...
当前第七步 `lfm_precomputed.py` 已经临时承担了部分相近职责：它会生成 `lfm_layer_control_points.csv`。第二轮重构后，这部分井空间事实和分频事实应前移到本步骤；第七步只消费这些事实并做顺层插值建模。
...
`mode: diagnose` 时，脚本在候选 cutoff 上比较低频曲线正演、第五步批量合成质量、井上高频 residual 能量和边界稳定性，选出全窗默认分频。`mode: manual` 时，跳过诊断，直接使用用户指定 cutoff。无论哪种模式，最终 cutoff、滤波阶数、缓冲策略和诊断证据都要写入 `run_summary.json`。
...
统计项至少应覆盖：高频 residual RMS、绝对值分位数、正负尾部分布、事件密度、正负状态持续长度、二状态转移矩阵、垂向自相关长度、反射系数量级和频谱能量。
...
第七步 LFM 应从本步骤读取共享井空间事实或低频控制事实，再做顺层切片插值。当前 `lfm_precomputed.py` 自己生成 `lfm_layer_control_points.csv` 是过渡实现；第二轮后它应尽量只承担 LFM 建模职责。
