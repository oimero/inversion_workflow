...
well_constraints:
  ...
  frequency_bands:
    ...
    ginn:
      target_source: frequency_lowpass  # frequency_lowpass | auto_tie_filtered_las
      mode: diagnose
      manual_cutoff_hz: null
...
`target_source` 控制 GINN 井目标：

- `frequency_lowpass`（默认）：使用诊断 cutoff 的 TWT 域低通曲线。
- `auto_tie_filtered_las`：读取第四步 `filtered_las_file`，按 optimized TDT 直接投影到 TWT；不再做 Hampel、额外 Gaussian 或 Butterworth 处理。

两种模式都会执行 `diagnose` 正演扫描。filtered 模式下诊断 cutoff 只用于 reference cutoff 和同窗对照，不定义 GINN target。

诊断表示"GINN 能从地震里稳定恢复到的最高频率"：

1. 对每个候选 cutoff，取参考井 log-AI，低通后正演合成记录，与真实地震道比较相关性和误差。
2. `scale<=0` 的候选视为极性不一致，不进入聚合。
3. 先在每个空间簇内取井指标中位数（防止密井平台垄断投票），再跨空间簇取中位数。
4. 找到全局最佳相关系数的近邻平台，在平台内选**最低的** cutoff。
5. 如果选中值落在候选边界上，脚本可主动失败，要求扩展候选范围。
...
ginn_target_log_ai = LP(conditioned_log_ai, f_ginn)
                     或 project(filtered_las, optimized_tdt)
...
Reference/LFM 的长缺口段不参与滤波。filtered target 保留第四步曲线的连续有效段，直接投影后与 reference/LFM 取有限值交集。
...
| `ginn_target_ai` / `ginn_target_log_ai` | 配置选择的 GINN 目标 |
| `ginn_band_log_ai` | target − LFM；filtered 模式下是经验增量而非严格中频 |
...
| `log_ai_anchor_time.npz` | `ginn_log_ai_anchor_v2`；记录所选 target source/semantics 的 GINN 井 anchor | GINN 训练 |
...
| `ginn_target_comparison.csv` | 同窗比较 selected lowpass 与 filtered LAS target 的 corr/NMAE/scale |
