...
网络不直接重画一套 AI 体，而是在第六步低频模型的基础上学习一个相对扰动。这样做的直觉是：LFM 负责大尺度趋势，神经网络只补目标层内能被地震波形约束的高频细节。
...
网络按道处理，每次看一条地震道上的时间序列。默认输入是三类信息：地震道、低频 AI 和目标层 mask；输出是这条道上的高频扰动。
...
训练目标有三层含义：先让合成记录贴近观测地震，再约束扰动不要离 LFM 太远，最后抑制不合理的层内振铃。
