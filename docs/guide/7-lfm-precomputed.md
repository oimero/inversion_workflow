...
`boundary_extension_samples` 控制目标层外的上下缓冲。第八步训练和物理正演会受到子波长度、mask 边界和卷积边界影响；如果 LFM 只在目标层内有值，边界附近容易出现突变。第七步会在目标层上方和下方各扩展一段样点，用相邻层段的边界趋势延拓，让第八步读取到的体在边界附近更平稳。
