...
`real_field_lfm.py` 是工作流的第七步。它从第六步冻结的井控集出发，按配置中显式声明的基线模型、修饰器和变体图，一次性构建所有请求的低频模型变体。

这是统一低频模型 v2 架构的核心：**同一套构建器在时间域和深度域之间共享，框架是可选修饰器而非第三种基线模型。** 时间域与深度域的差异只体现在 cutoff 单位、采样轴单位和 source adapter——构建器和修饰器内部逻辑完全相同。
