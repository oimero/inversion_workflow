...
`ginn_inversion.py` 是时间域工作流的第九步。它读取第八步训练好的 checkpoint，复用 checkpoint 内保存的训练配置，在全目标层范围内推理并输出 stage-1 波阻抗体。
...
| 输入 | 用途 |
|------|------|
| 第八步 checkpoint | 重建训练配置、加载模型权重和事实链 |
...
checkpoint 内已经包含第八步训练配置，因此第九步不应该重复配置 `ai_lfm_file`、`wavelet_file` 或地震路径。除非用户显式传 `--checkpoint`，脚本只使用配置里的 checkpoint。
...
3. 初始化 `src.ginn.trainer.Trainer`，复用第八步的数据加载、mask、LFM、子波和 fixed gain 口径。
4. 加载模型权重，调用 `predict_volume()` 得到全目标层 stage-1 波阻抗。
5. 未进入 inference mask 的样点保持 LFM，不静默留 0。
6. 输出 NPZ、可选 SEG-Y、QC 图和统计摘要。

第九步不是新的训练实验入口。它只消费第八步 checkpoint 的事实链，保证反演使用的 seismic、LFM、wavelet、mask 和 fixed gain 与训练一致。
...
| 原因 | 含义 | 怎么处理 |
|------|------|---------|
| `Checkpoint not found` | 配置或 CLI 指向的 checkpoint 不存在 | 检查第八步输出目录和 `checkpoint_path` |
| `model_state_dict` 缺失 | checkpoint 不是训练脚本保存的模型文件 | 使用 `best.pt` 或 `last.pt` |
| `LFM shape does not match seismic shape` | checkpoint 配置指向的 LFM 与地震不一致 | 回到第八步确认训练配置是否是当前工区 |
