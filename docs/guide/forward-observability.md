...
`forward_observability.py` 是稳定生产链（第一至五步）之后持续运行的时间域研究旁路。它回答一个具体问题：**在当前全局子波、井震匹配和目标时窗下，某个频率的波阻抗扰动能否在地震上产生可辨识的响应。** 它不占用工作流步骤编号，也不是主流程门禁；输出只用于可选的 `synthoseis-lite` probe benchmark 和 R0/R1 QC，不进入反演模型、训练分布、损失权重或正演校准。

时间域工区始终可以运行该旁路。当前实现的 FFT、频率探针和有限差分分析均保留；延期的是深度域 Jacobian/SVD 扩展，而不是时间域旁路本身。
...
log(AI) 上的有限差分步长，默认 0.001。这个值在"太大会扭曲线性化近似"和"太小会让浮点噪声淹没梯度"之间折中。如果你的 AI 绝对值范围很窄（< 3 GPa·s/m），可以考虑略微增大。
...
人工扰动场景的幅度。脚本从标称子波出发，生成四个受控扰动：±10° 相位旋转（`artificial_phase`）和 ±0.5 样本分数时间偏移（`artificial_shift`）。这些扰动通过零填充的解析信号/傅里叶斜坡实现，不存在周期性边缘缠绕。
...
log(AI) 曲线投影到 TWT 轴后，允许线性插值填补的短间隙最大长度（秒），默认 0.010 s。超过此长度的间隙不填补，整个窗口被跳过。
...
1. 通过优化 TDT 将滤波后的 log(AI)（第四步）和预处理后的 log(AI)（第三步）从 MD 投影到 TWT 轴。短间隙（≤ `max_short_log_gap_s`）做线性插值填补。
2. 从有序层位构建全目标窗口和邻区窗口。
...
- **正演与失配基准：** 对滤波后的 log(AI) 做 Robinson 正演（`tanh` 反射率 + `numpy.convolve(mode="same")`，挂在下部样本），在 Tukey 加权下做观测值中心化/标准化后的正尺度零截距振幅拟合。输出 `corr`、`nmae` 和 `mismatch_rms`。
...
这些区间不是模型截止频率或生产频带边界，仅用于 `synthoseis-lite` 的设计。
...
- `figures/wells/well_observability_<well>.png` — 每口可分析井的三面板 QC 图：第三步 vs 第四步 log(AI) 对比、观测地震道、单井场景 P25 可检测比 vs 频率。
...
`missing_input` 的井通常是因为第四步标定失败或预处理未通过；`missing_horizon` 说明该井缺少某个配置层位的 MD 拾取值；`outside_tdt_support` 说明层位的 MD 超出了时深表覆盖范围。
...
- 左图（log AI）：第四步滤波后的 log(AI) 应该平滑地跟随第三步预处理 log(AI) 的趋势，不应出现大幅偏离。
...
| `configured_max_beyond_wavelet_support` | 配置的 `max_hz` 超过标称子波 1.5 倍右半幅频率 | `run_summary.json` 的 `warnings` 包含此信息；不是致命错误，但高频分析不可靠 |
