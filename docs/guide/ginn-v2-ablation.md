...
- `benchmark_dir` 解析后的绝对路径和 `synthetic_benchmark.h5`、`sample_index.csv`、`benchmark_manifest.json` 三文件的 SHA-256 均写入 `model_run_manifest.json`，保证结果可审计。
...
控制真实工区井数据的稀疏 delta 监督。这是普通 GINN-v2 loss 分量，不是独立工作流步骤。
...
盖章后 manifest 中新增 `synthetic_gate_evidence_status: "ok"` 和完整的 `synthetic_gate_evidence` 记录（含 report_dir、report_card 路径和 report_card 的 SHA-256）。这是后续真实工区 R0 推理的准入门槛——manifest 中缺少合成闸门证据的模型不会被 `real_field_zero_shot.py` 接受。
...
比较两次实验前，应核对 manifest 中的以下指纹一致：
- `benchmark_hashes`（三个文件的 SHA-256）
- `patch_index_sha256` 和 `normalization_sha256`
- `patch_spec` 和 `split` 配置
- `training.synthetic_sequence_sha256`

这样锁定的是实验条件，不是路径依赖。如果以上指纹全部一致，两次运行的唯一差异来自 `lambda_real_delta`（0.0 vs 0.1）——即 control vs anchor 的唯一定量差异。
