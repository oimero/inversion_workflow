...
**`lambda_real_delta`**：非零时在每个 training step 中额外采样显式变体的 `well_log_ai - lfm_log_ai` normalized-delta MSE。权重大于 0 时必须配置 `train.real_delta` 段，且当前只支持 `no_lateral` 角色。设为 0.0 但保留 `real_delta` 段时，不参与训练但仍生成 best/final 全井 QC（QC-enabled control）。
...
三者必须显式提供，不接受 `auto`。解析时依次校验成功的 `real_field_lfm_run_v3`、`variant_manifest.csv`、所选 `real_field_lfm_variant_v3` 主 NPZ，以及第6步的显式数据模式/轴/形状/掩码语义。所选变体和井控集的直接契约指纹写入模型 manifest 的 `input_contracts`。
...
每个选中井的 `well_log_ai - lfm_log_ai`（来自第6步 canonical 控制和所选第7步变体三线性插值）作为 normalized-delta target。每口井的选择次数记录在 `real_delta_sampling_qc.csv` 中；变体 ID、低频模型变体契约指纹和井控集契约指纹写入样点契约。

#### 可微井预测器

`DifferentiableWellPredictor` 在不含横向混合的模型（`receptive_field_lateral == 1`）上使用 sparse 模式：只对井轨迹穿过的线号/道号节点做逐道逐块推理，再通过三线性插值得到井轨迹上的预测值。
...
这些变换在加载真实工区地震体和低频模型体时应用，确保输入分布与训练时的合成数据一致。`input_reference_stats.json` 由训练阶段自动生成，记录训练集上地震数据的 P01/P05/P50/P95/P99/abs_p95/abs_p99 等参考统计量。
...
1. **加载基准**：读取 benchmark 的直接契约指纹，并校验数据模式、domain、形状、数据类型、采样轴和掩码等显式语义；不重算 benchmark 文件 SHA。
...
Manifest 数据模式为不兼容旧版本的 `ginn_v2_model_run_v3`。下游默认读取 manifest 中
`checkpoints.primary=best`；旧 v2 及更早运行必须重训。手工预测可用
...
best/final 井 QC 图。它们只陈述当前 run 的绝对证据。
