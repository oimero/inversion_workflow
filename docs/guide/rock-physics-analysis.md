...
2. 逐一检查每口井的 `preprocessed_las` 路径是否存在且可读取；记录路径和曲线清单，不扩散逐 LAS 哈希。
...
岩石物理 run 只发布一个 `contract_fingerprint_sha256`。下游复制这个直接上游指纹，不重新读取 LAS、关系文件或子波计算哈希；domain/unit、采样轴和子波中心等仍由显式语义校验负责。
...
如果后续 Synthoseis 报契约不匹配，核对岩石物理 run 与校准/生成 manifest 的 `input_contracts`。旧 schema 不兼容；上游 LAS、关系或子波发生变化时，重建岩石物理 run 及其下游。
