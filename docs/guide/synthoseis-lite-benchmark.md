...
深度域不使用 `expected_output_dt_s`，模型网格的纵轴是 TVDSS（米），分辨率由工区原生采样决定。
...
定义需要生成剖面的空间路径，每条路径是若干 inline/xline 点连成的折线：
...
**1) 收集井数据。** 从上游产物中读取每口标定成功井的测井曲线（时间域读第四步的滤波 LAS，深度域读 Step 5 的平移 LAS），结合井分层和解释层位，提取每口井在目标层段内的波阻抗曲线。
...
- **坐标体系**：纵轴是 TVDSS（米）而非 TWT（秒），模型网格分辨率由工区原生采样决定。
