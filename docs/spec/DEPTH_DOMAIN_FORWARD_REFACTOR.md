...
> SHA-256 注意：本文的逐文件哈希与 `forward_model_inputs_sha256` 内容属于历史设计，
> 已由 [SHA-256 契约瘦身规范](SHA256_CONTRACT_SLIMMING.md) 取代。现行 schema 只发布
> 单一契约指纹并记录直接上游；旧 run 必须重建，不提供哈希字段兼容。
