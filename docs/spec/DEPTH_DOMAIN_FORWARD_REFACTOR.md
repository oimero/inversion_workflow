...
在 `src/cup/physics/` 建立统一物理能力模块：同时支持时间域和深度域的正演内核，并承载域无关的岩石物理关系，供 Step 6、Synthoseis-lite、GINN v2 和 R1 共同使用。正演可观测性分析是非阻塞旁路，不是统一正演内核及其下游的前置门禁；时间域工区持续运行既有 FFT/有限差分旁路，延期的只是深度域扩展。
...
Step 1—3 已完成，主要负责数据盘点、曲线筛选和测井预处理，不引入正演。当前深度工区正式采用工区专用的旧 Step 4/5：Step 4 从 NW11 提取固定时间子波；Step 5 用该子波完成七口井的批量合成、时移扫描和 shifted LAS 输出。两个脚本保留在 `scripts/`，但不迁入公共 API。新 Step 6 独立消费 Step 3 预处理 LAS，通过可开关模块拟合全工区 AI—Vp 关系；Step 4/5 的井震标定与 shifted LAS 不参与该拟合。
...
- 当前工区的旧 Step 4/5 已在深度域实践中验证，其 NW11 子波、七井时移和 shifted LAS 是正式工区产物，但不构成岩石物理 Step 6 的 LAS 数据源。
...
该关系由新 Step 6 在 Step 3 规则 MD 测井样点上拟合，数学上不依赖地震采样域，也不得为了拟合投影到 TWT。实井正演优先使用实测 Vp 和 Rho 构造 AI，同时把实测 Vp 作为传播速度；冻结的全工区 AI—Vp 关系主要服务于仅预测 logAI 的合成数据与模型推理闭环。
...
flowchart TD
    ...
    B --> C
    A --> D["Step 6 输入层：发现并读取全部 passed LAS"]
    D --> I["well_input_inventory.csv"]
    I --> M["可开关模块：ai_vp_linear"]
    M -->|enabled 且成功| RP["全工区 AI–Vp 关系"]
    M -->|disabled| U["run_summary.json：成功空跑"]
    RP --> E["forward_model_inputs.json"]
    B -.->|显式 NW11 子波文件| E
    E --> F["cup.physics"]
...
主路径不再依赖正演可观测性报告。Step 6 的 LAS 输入只来自 Step 3；井分层、Step 4 标定结果和 Step 5 shifted LAS 均不参与 AI—Vp 拟合。只有 `ai_vp_linear` 模块启用且成功时，Step 6 才把关系与显式 NW11 子波装配成 `forward_model_inputs.json`，供统一正演、合成数据、训练和 R1 使用。
...
- 不把该工区专用脚本迁入 `cup.physics` 或通用井震标定接口。

需要 NW11 子波的后续步骤必须接收显式子波文件，禁止按文件修改时间猜测或回退到其他井的子波。
...
Step 5 必须保留每口成功井的 shifted LAS、时移结果、合成地震和 QC。其七井集合只描述本步骤的成功结果，不限制岩石物理井群；`L3-NW2A`、`L6-NW3A` 若在 Step 3 状态为 `passed`，仍进入 Step 6 候选集。

### 8.3 Step 6：岩石物理分析

入口为 `scripts/rock_physics_analysis.py`。该脚本由始终执行的输入层、可开关分析模块和条件式正演清单装配层组成；自身不执行地震正演。
...
rock_physics_analysis:
  ...
  forward_model:
    wavelet_file: scripts/output/vertical_well_auto_tie_depth_NW11/wavelet_201ms_NW11.csv
    source_well: NW11
```

输入层规则：

- 使用 `resolve_source_run()` 自动发现最新且包含 `well_preprocess_status.csv` 的合格 `well_preprocess_*` run；`source_runs.well_preprocess_dir` 非空时严格使用显式目录；
- 以 `well_preprocess_status.csv` 为权威井清单，读取全部 `preprocess_status=passed` 的 `preprocessed_las`；
- 状态表声明为 `passed` 但 LAS 缺失、无法读取或路径重复时，输入契约破坏，Step 6 整步失败；
- 顶层始终写出 `well_input_inventory.csv`，记录全部状态行、入选状态、LAS 路径和 SHA-256、读取状态、曲线名及单位；
- 不读取井分层、Step 4 标定结果、Step 5 指标或 shifted LAS。

模块框架规则：

- `modules.ai_vp_linear.enabled` 必须显式配置；缺失 `enabled`、未知模块名或非法模块配置立即失败；
- `enabled=false` 时，输入层仍完整读取所有 passed LAS，随后以成功状态结束；只生成 `well_input_inventory.csv` 和 `run_summary.json`；
- 模块关闭时 `forward_model` 段可以缺失，且不得读取或校验子波，不创建模块目录、关系产物、图件或 `forward_model_inputs.json`；
- 模块启用时，`forward_model.wavelet_file` 和 `source_well` 必须为非空显式配置；只在拟合成功后才检查并读取子波文件；
- 模块启用时，模块级数据问题逐井拒绝并写明原因；剩余合格井少于 `min_valid_wells=3` 时模块和整步失败。

`ai_vp_linear` 数据规则：

- 从每口 Step 3 LAS 读取 `DT_USM [us/m]`、`RHO_GCC [g/cm3]`、`AI [m/s*g/cm3]`；
- 显式派生 `Vp = 1_000_000 / DT_USM`，并重算 `AI_recomputed = Vp * RHO_GCC`；LAS 中的 `AI` 仅作上游一致性 QC，不直接作为拟合输入；
- 在 DT、Rho、LAS AI 和重算 AI 共同有限且物理值为正的样点上比较 AI；固定使用 `rtol=1e-5`、`atol=1e-3 m/s*g/cm3`；超差井以 `ai_consistency_mismatch` 拒绝；
- 不新增填值、异常值裁剪、重采样或平滑；
- 每口井至少保留 100 个有限正值样点，否则该井以 `insufficient_valid_samples` 被模块拒绝；
- 拟合使用 Step 3 规则 MD 网格上的原生测井值，不投影到 TWT。MD 与 TVDSS 的常量基准差不改变参与拟合的 `(Vp, AI)` 数值对。
...
所有模块合格井具有相同总权重；井内每个有效样点均分该井权重。因此长井和高采样密度井不能凭样点数主导全工区关系。拟合采用等井权 Huber 回归：
...
逐井普通拟合仅用于 QC 对比；逐井或分层系数不得写入下游正演输入。模块启用并成功时生成：

- `modules/ai_vp_linear/rock_physics_relation.json`：全局 a/b、公式、单位、实际合格井清单、权重、Huber 参数、来源哈希和汇总 QC；
- `modules/ai_vp_linear/well_fit_qc.csv`：全部候选井的模块状态、样点数、Vp/AI 值域、R²、RMSE、MAE、偏差和拒绝原因；
- `modules/ai_vp_linear/figures/ai_vp_fit.png`：分井散点或密度、全局直线和残差；
- 顶层 `forward_model_inputs.json`：显式 NW11 子波、全局关系、Step 3 来源、TVDSS/domain 契约及各自哈希；
- 顶层 `run_summary.json`：输入发现方式、模块启停/状态、产物、拒绝统计和运行状态。

输入层契约破坏、模块配置非法或全局拟合校验失败都会导致整步失败。模块级曲线缺失、单位错误、AI 不一致、样点不足或非正值按井拒绝；模块至少三口井成功且全局拟合校验通过时，整步成功并生成正演清单。
...
顶层 `well_input_inventory.csv` 始终生成，至少包含 `well_id`、`preprocess_status`、`selected_for_input`、`las_path`、`las_sha256`、`read_status`、`curve_names`、`curve_units` 和 `reasons`。它审计输入层实际读取了哪些 Step 3 LAS，不包含模块拟合结论。

`modules/ai_vp_linear/well_fit_qc.csv` 使用固定列契约，不凭字段名猜单位；至少包含 `well_id`、`module_status`、`reasons`、`n_samples`、`vp_min_mps`、`vp_max_mps`、`ai_min_mps_gcc`、`ai_max_mps_gcc`、`ai_consistency_max_abs_mps_gcc`、`ai_consistency_max_rel`、`r2`、`rmse_mps_gcc`、`mae_mps_gcc`、`bias_mps_gcc` 和 `well_total_weight`。

`run_summary.json` 至少记录：

- schema、脚本版本和运行状态；
- Step 3 来源目录、`auto_discovered|explicit` 发现方式及状态表路径；
- 输入层 passed 井、成功读取井和输入契约错误统计；
- 已知模块清单及每个模块的 `enabled|disabled`、`success|failed` 状态；
- 模块候选井、成功井和逐原因拒绝统计；
- 每个输入文件与产物的路径和 SHA-256；
- 样点拒绝原因及数量；
- 启用成功时的关系文件、显式子波和 `forward_model_inputs.json` 哈希。

全部模块显式关闭时，运行状态为成功，并明确记录 `no_analysis_modules_enabled`；除输入清单和摘要外不生成其他产物。失败运行可以写诊断摘要，但不得写出状态为可用的关系或正演输入清单。
...
1. 固定当前已验证的 NW11 Step 4 子波和七井 Step 5 QC/shifted-LAS 产物；二者不作为 Step 6 的 LAS 拟合输入。
2. 为当前时间域 NumPy Robinson 正演固化有限、奇数子波的数值 fixture。
3. 为 `src/ginn_depth/physics.py` 的旧深度矩阵固化小规模数值 fixture。

门禁：Step 4/5 来源可追溯、职责边界明确；fixture 可独立加载，且不通过导入遗留实现实时生成期望值。

### 阶段 2：新 Step 6 岩石物理分析

1. 实现 Step 3 run 自动发现/显式覆盖、passed LAS 严格读取和顶层输入清单。
2. 实现显式模块注册、`enabled` 校验及无模块成功空跑路径。
3. 实现 DT/Rho 派生、LAS AI 一致性 QC、逐井拒绝、等井权最小二乘初值、加权 MAD 和 Huber 拟合。
4. 将模块产物写入 `modules/ai_vp_linear/`；成功后才读取显式 NW11 子波并装配正演输入清单。

门禁：模块关闭路径只生成输入清单和摘要；模块开启路径至少三井成功，且 a/b、单位、AI 一致性、正速度、等井权和来源哈希校验通过；井顺序不影响结果。
...
1. Step 6 默认自动发现最新合格 Step 3 run；显式 `well_preprocess_dir` 覆盖自动结果。
2. `well_preprocess_status.csv` 中全部 passed 井进入输入候选集，井顺序不影响输入清单或模块结果。
3. passed LAS 缺失、损坏、重复路径或无法读取时，即使模块关闭也整步失败。
4. `enabled=false` 时只生成 `well_input_inventory.csv` 和成功摘要，不读取子波、不创建模块目录或正演清单。
5. 未知模块名、缺失 `enabled` 和非法模块参数明确失败。
6. Vp 严格由 `1e6/DT_USM` 派生，AI 严格由 Vp·Rho 重算；单位错误不被猜测或换算。
7. LAS AI 在 `rtol=1e-5`、`atol=1e-3 m/s*g/cm3` 内闭合；超差井以 `ai_consistency_mismatch` 拒绝。
8. 模块不填值、不裁异常、不平滑、不重采样；少于 100 点的井被拒绝，少于三口合格井时模块失败且无正演清单。
9. 每口合格井归一化总权重相同；单独复制一口井的样点不增加该井总影响。
10. Huber 拟合面对单井少量极端残差时比等井权最小二乘稳定，并记录稳健权重变化。
11. 相同 `(Vp, AI)` 数值对仅改变 MD/TVDSS 坐标标签或常量基准时，拟合结果不变。
12. 公式、单位、`a > 0` 和全部拟合样点反算正速度校验正确。
13. 模块成功后才要求显式 NW11 子波；正演清单精确引用 Step 3、输入清单、关系和子波哈希。
14. 修改任一输入 LAS、关系文件或子波时，相关来源哈希及 `forward_model_inputs_sha256` 随之改变。
15. 新时间 NumPy 实现与当前 Robinson 正演逐点一致。
16. NumPy/PyTorch 时间和深度结果分别一致。
17. 分块深度结果与完整旧 `W_depth` fixture 一致；`return_operator` 两条路径 shape 正确。
18. PyTorch 对 logAI 和速度分别通过双精度 `gradcheck`。
19. 常速度模型满足解析相对 TWT、界面时间和矩阵权重预期。
20. 子波线性插值在节点、节点中点和支撑区外行为正确。
21. 非有限值、非正速度、非递增深度、偶数/非规则子波、错域和错单位输入均明确失败。
22. time/depth 两套 Synthoseis-lite 小型端到端闭合；深度高分辨率正演、抗混叠和 5 m 降采样闭合。
23. Synthoseis 和训练在没有可观测性产物时运行；添加 probe 只增加隔离评估产物，不改变训练数组或其哈希。
24. 数据集、模型、R1 对旧 schema 和错正演输入哈希明确失败。
25. R1 深度合成、观测与 TVDSS 轴逐点严格一致且均为 `N` 点；相位扰动与米制深度平移相互独立。
26. inline 步长 1、xline 步长 4 的体采样集成测试。
...
- 新 Step 6 自动发现或显式固定 Step 3 run，读取全部 passed LAS，且不消费井分层、Step 4 标定结果或 Step 5 shifted LAS；
- 模块启停必须显式；全部模块关闭时成功生成输入清单和摘要，但不生成关系或正演清单；
- `ai_vp_linear` 启用时，DT/Rho 派生、AI 一致性、逐井拒绝、至少三井、等井权 Huber、a/b、正速度和全部来源 SHA-256 可追溯；
- 模块成功后才读取显式 NW11 子波，并生成引用 Step 3 输入、关系和子波哈希的 `forward_model_inputs.json`；
...
| 风险 | 表现 | 控制 |
|---|---|---|
| 自动发现漂移 | 新 Step 3 run 改变拟合井群 | 记录 `auto_discovered|explicit`、run 路径、状态表和逐 LAS SHA-256；允许显式覆盖 |
| passed 产物损坏 | 状态表合格但 LAS 缺失或不可读 | 输入层整步失败，不受模块开关影响 |
| 模块被意外关闭 | 漏配置被当成合法空跑 | 模块名白名单；`enabled` 必填；未知或缺失配置失败 |
| 模块关闭仍产生旧校准 | 下游误用残留关系或正演清单 | disabled 路径只写输入清单和摘要，不创建关系/manifest |
| Step 3 AI 与重算值不一致 | 上游导出契约损坏后仍进入拟合 | 固定单位和容差；超差井显式拒绝 |
| 长井或密采样井主导回归 | 全局关系实际代表单井 | 每井总权重相同，井内均分；样点复制与井顺序不变性测试 |
| 少量异常点扭曲 a/b | 派生速度和深度算子系统性偏差 | 等井权 Huber、加权 MAD、逐井残差 QC |
| 拒绝井过多 | 少量井代表全工区 | 每井至少 100 点、模块至少三井；逐原因拒绝统计 |
...
该设计的核心边界是：岩石物理关系从 Step 3 原生测井值拟合，本身不区分时间域与深度域，也不受井分层或井震标定平移影响；时间子波仍属于时间坐标，地震输出属于声明的采样域；速度场通过相对旅行时把二者连接起来。自动发现只负责选择 Step 3 run，实际 run、井清单、单位和逐文件哈希必须完整记录。任何试图用样点数、文件名或未记录的“最新运行”替代坐标、单位、domain 和来源哈希契约的实现，都不符合本规范。
