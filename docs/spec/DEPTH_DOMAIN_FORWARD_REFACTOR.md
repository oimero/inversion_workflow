...
在 `src/cup/forward/` 建立同时支持时间域和深度域的统一正演能力，供可观测性分析、Synthoseis-lite、GINN v2 和 R1 共同使用。
...
Step 1—3 已完成，主要负责数据盘点、曲线筛选和测井预处理，不引入正演。当前深度工区不建设标准 Step 4/5：固定采用一次性脚本从 NW11 得到的时间子波。该子波是后续正演的显式输入产物，不把一次性脚本迁入公共 API。
...
| `src/wtie/modeling/modeling.py` | 裸卷积；偶数子波会自动补零 | 一次性深度脚本 | `wtie` 保持不动；新核心禁止复制该兜底行为 |
...
- 当前工区用一次性深度脚本从 NW11 提取固定时间子波；该过程不属于待迁移的标准工作流。
...
```mermaid
flowchart LR
    A["Step 3：Vp / Rho / TVDSS"] --> B["一次性脚本：固定 NW11 时间子波"]
    A --> F["Step 6：深度 Jacobian / SVD"]
    B --> F
    A --> G["Synthoseis-lite v2"]
    B --> G
...
NW11 子波必须记录文件路径、SHA-256、秒制时间轴和来源脚本。AI—Vp 关系不在本阶段拟合；需要耦合速度的调用方必须从显式配置读取 a/b，并把配置与子波共同纳入正演输入哈希。
...
2. **AI—速度耦合敏感度**：由显式配置 a/b 将扰动后的 AI 转为 Vp，同时改变反射率与 `W_depth`。

实现使用 PyTorch autograd 构造局部 Jacobian，并对其做 SVD 或等价的局部谱分析。随机抽取若干列，用中心有限差分复核 autograd 结果。

深度轴探针使用 `cycles/km` 的深度波数；Hz 仅用于描述时间子波的频谱，不得把 Hz 直接解释为深度数据的轴向分辨率。报告应明确分析井、TVDSS 窗口、线性化点、固定/耦合模式、正演输入哈希和奇异值截断规则。
...
- 主监督目标保持 logAI。
- 深度域 physics loss 使用预测 logAI 和显式配置 a/b 派生 Vp，再调用 `forward_depth`。
- 时间域 physics loss 调用 `forward_time`。
- 模型运行 manifest 必须记录 `sample_domain`、轴 basis、正演正演输入哈希和 synthoseis schema。
- 训练时发现数据集、配置、checkpoint 三者 domain 或正演输入哈希不一致，立即停止。
...
1. 读取 R0 预测的 logAI、明确的 TVDSS 轴和正演输入哈希；
2. 使用完全相同的显式配置 a/b 派生 Vp；
...
### 9.1 `forward_model_inputs.json`

该文件是显式输入清单，不由标准工作流自动拟合或生成。最小结构：
...
```json
{
  ...
  "sources": [
    {"path": "scripts/vertical_well_auto_tie_depth.py", "sha256": ""}
  ]
}
```

实际使用前必须填写真实值。文件自身 SHA-256 作为 `forward_model_inputs_sha256` 写入下游产物。一次性脚本只负责产生 NW11 子波，不负责拟合 a/b 或自动组装该清单。
...
2. `WorkflowConfig` 增加 domain/depth_basis 严格配置。
3. 实现 NumPy/PyTorch 时间内核、相对 TWT、线性子波插值和完整深度算子。
4. 实现默认 64 点输出分块路径、grid 适配器和 AI—Vp 变换。
5. 固定 NW11 子波及来源哈希，作为测试和后续运行的显式输入。

门禁：时间 fixture、旧深度 fixture、后端一致性和 gradcheck 全部通过；NW11 子波路径、时间单位和哈希可追溯。
...
11. Step 6 Jacobian 与中心有限差分抽查一致。
12. time/depth 两套 Synthoseis-lite 小型端到端闭合。
13. 深度高分辨率正演、抗混叠、5 m 降采样路径闭合。
14. 数据集、模型、R1 对旧 schema 和错正演输入哈希明确失败。
15. R1 深度合成、观测与 TVDSS 轴逐点严格一致，三者均为 `N` 点。
16. 相位扰动只改变时间子波，深度平移只使用米制参数。
17. inline 步长 1、xline 步长 4 的体采样集成测试。
...
| AI—Vp 显式参数错误 | 派生速度偏向单井 | 显式单位、正斜率和派生正速度校验 |
