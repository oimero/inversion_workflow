...
Step 6 的 LAS 输入只来自 Step 3，不依赖井分层或 Step 4/5 标定结果。
...
Step 7 深度域 LFM 独立规范见 [`DEPTH_DOMAIN_FRAMEWORK_LFM.md`](DEPTH_DOMAIN_FRAMEWORK_LFM.md)，当前待实施。R1 用冻结 a/b 和子波在 TVDSS 轴上生成 N 点合成深度地震；合成与观测轴逐点一致；相位扰动和米制深度平移独立扫描。
...
### 阶段 4：Step 7 深度域框架约束 LFM（待实施）

按 [`DEPTH_DOMAIN_FRAMEWORK_LFM.md`](DEPTH_DOMAIN_FRAMEWORK_LFM.md) 实施。门禁：每个比例切片至少三口控制井；M0/M1 同网格同掩码、来源可追溯。

### 阶段 5：R0/R1 与重复实现清理（待实施）
