...
用户提供的 `scripts/output/real_field_zero_shot_20260714_194343` 是在本轮 R0 schema
切换前生成的 `real_field_zero_shot_summary_v4` 产物。它保留为历史结果，不作为当前
R1 输入；当前 reader 要求 v5 canonical prediction 字段，因此必须用新 checkpoint
重新执行 R0 后才能进行真实工区 R1 收尾。

本轮收口修复还包括：正权重监督块才可建立 physics 部署资格；physics stage 的
final checkpoint 固定为诊断产物；深度 R1 的 `lfm_only` 井 QC 直接使用外部 LFM，
不再伪装成模型预测；R0 中不可达的 external target-mask 读取分支已删除。
...
- [x] 按清单退役被替代的旧生产代码，历史产物由冻结目录和 Git 历史保证可复现。
  ...
  `LEGACY_KEYS` 和 R0 旧别名集合仅保留为明确失败入口，不作为新训练或新预测字段；
  它们不是兼容实现。
...
- [x] physics checkpoint 的部署资格按正权重 dense synthetic supervised 指标控制，且 physics stage 仅允许 best checkpoint 部署；
- [x] R0 有效点全覆盖且无有效点 NaN；
- [x] R1、LFM-only、canonical closure 和 deployment closure 字段分开；深度域真实剖面 smoke 已完成。
...
阶段 9 的代码门禁、synthetic physics fixture、R0/R1 schema 和真实剖面 R1 smoke 已落地。
体模式 R1 可在需要时复跑；阶段 10–12 的微纹理与 paired A/B/C 继续作为附加实验，
不阻塞当前 canonical increment 工作流。每完成一项就在对应复选框勾选并记录测试命令、
fixture 和产物路径。
...
混合 synthetic-supervised + real-physics 阶段完成后，部署评估索引按
`validation.selection_metric` 指向的 synthetic block 显式选择。manifest 同时记录每个
loss block 的 patch index；合成预测入口会在加载 checkpoint 前验证 index 字段合同。
部署资格中的 synthetic anchor 必须为正权重且每步参与训练。

深度域 R1 与 Synthoseis-lite 共用 `DepthForwardExecutor`。`auto` 在 CUDA 可用时使用
Torch float64 backend，并按相同有效深度段批量正演；零相位和零时间移位扫描复用 base
结果。R1 记录可读的 R0 summary 与 `forward_model_inputs` 路径，不为这些路径递归计算
SHA-256。

实际深度剖面 smoke：

```text
scripts/output/real_field_forward_diagnostic_gpu_smoke_20260715_r4
```

结果为 `status=ok`，schema 为 `real_field_forward_diagnostic_summary_v6`，forward contract
记录 `resolved_backend=torch_cuda`、`dtype=float64`、`batch_size=64`。CPU/Torch float64
数值对照最大绝对差约 `4.63e-16`；按 551 道、256 样点的同形 batch 基准，CUDA 正演约
为 NumPy 的 10–16 倍。相关本地回归结果为 `55 passed`，`compileall` 和 `git diff --check`
通过；测试文件仍遵守仓库 `.gitignore`。
