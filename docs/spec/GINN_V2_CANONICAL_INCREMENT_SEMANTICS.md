...
2. 同时包含 physics 和每步参与优化的正权重 `synthetic_supervised` block、从监督 checkpoint 初始化，并使用该 synthetic block 的 `<block_id>.mse` 作为 selection metric 的阶段 **best**。用于部署资格的 synthetic block 固定 `update_interval: 1`；该阶段的 final checkpoint 只作诊断，不能部署。
