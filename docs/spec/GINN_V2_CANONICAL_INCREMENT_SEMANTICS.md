...
滤波输入使用 float64。采样轴必须严格递增且等间隔，以首个间隔作为 `sample_step`，并满足：

```text
allclose(diff(sample_axis), sample_step, rtol=1e-6, atol=1e-9 axis units)
```

设计阶数 6 产生 3 个 SOS section。最小连续有限段长度固定为：

```text
minimum_segment_samples = 3 * (2 * n_sos + 1) = 21
pad_samples = ceil(buffer_axis_units / sample_step)
```

reflect buffer 还要求 `pad_samples < segment_samples`。任一条件不满足时明确失败，不切换滤波实现或边界模式。
...
log_base = natural
ai_unit_convention = m/s*g/cm3
canonical_lowpass_applied = true
canonical_lowpass_applied_to = well_controls_before_spatial_modeling
...
`canonical_lowpass_application_count` 表示该 LFM 生产链中的 canonical 低通次数。GINN v2 只接受值 1，并在消费后原样传递该字段。空间趋势拟合或 kriging 属于 LFM 构建过程，不增加该计数。

数组层面的频谱、空间突变和超低频响应只作为数值 QC，用于发现异常产物；这些指标不能证明数组由指定生产流程生成，也不参与硬合同判定。
...
2. 同时包含 physics 和至少一个监督 block、从监督 checkpoint 初始化，并使用该阶段监督 block 的 `<block_id>.mse` 作为 selection metric 的阶段 best/final。

独立 AI 或井上 gate 可以决定后续实验是否采用 physics 配方，但不能把 waveform-best checkpoint 直接提升为部署模型。首版不引入相对于前一 checkpoint 的 increment anchor。标准示例部署 `synthetic_supervised.best`。
...
2. 升级 Synthoseis-lite schema，生成共享标签的多 LFM variant benchmark，并让带限 controlled degradation 只作用于输入 LFM。
3. 升级 GINN v2 配置、reader、训练 batch、损失字段、checkpoint 和评估命名。
4. 将真实井监督目标切换为最终模型轴上的 canonical increment，并加入 physics 监督祖先和 deployment eligibility 校验。
5. 升级 R0/R1 产物、两类 closure 和反事实报告，保持全覆盖与显式坐标合同。
...
- 非等间隔轴和超出合同容差的采样间隔明确失败。
- 单程设计 cutoff 响应约为 −3.0103 dB，前后向最终响应约为 −6.0206 dB；设计 6 阶对应最终等效 12 阶。
- 设计 6 阶的最小连续段固定为 21 个样点，reflect buffer 必须短于连续段。
...
- LFM 消费端校验生产者元数据和 application count，不使用 `P(LFM) ~= LFM` 作为硬门禁。
...
- 含 supervised 与 physics block 的后续阶段只有按 supervised MSE 选优时才可部署。
