...
- ablation 的活动 reader 只读取同网格 seismic、canonical background、canonical
...
旧连续 ablation 的 reader、模型、checkpoint 和部署合同保持可运行。Structured
...
- `cup.synthetic` 不依赖 `ablation`；
- `ablation.structured` 可以依赖公开的 synthetic、projection 和 physics 接口；
- 新代码不复制时间域、深度域 decoder 或 HSMM；
- 域 Adapter 只提供轴名、单位、采样间隔和域专属 forward 参数；
- 旧连续 `ablation.models`、`ablation.data` 和 checkpoint schema 不改语义；
