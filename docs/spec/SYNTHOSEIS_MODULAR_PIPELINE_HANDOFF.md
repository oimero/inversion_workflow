...
> **2026-07-19 增补：** 上述“不新增空间变增益”是共享 Pipeline
> 重构的原始范围说明。活动科学合同使用真实地震与无视图 Pilot
> 共同标定的地震振幅先验，并由一个统一层序增益算子生成均值、随机残差和完整先验三个视图。
...
science revision: synthoseis_lite_science_v4
projection contract: finite_support_projection_v1
seismic view contract: seismic_views_v1
seismic operator contract: seismic_operators_v4
random stream contract: synthoseis_random_v3
```

science-v4 manifest 必须完整记录上述版本链。删除 science-v2 中的：
...
缺少任一 science-v4 字段或字段值不一致时直接失败。
...
seismic_views:
  operators:
    ...
    empirical_mean_gain:
      kind: calibrated_rgt_gain
      parameter_source: seismic_amplitude_prior
      mean_scale: 1.0
      residual_scale: 0.0

    empirical_residual_gain:
      kind: calibrated_rgt_gain
      parameter_source: seismic_amplitude_prior
      mean_scale: 0.0
      residual_scale: 1.0
  ...
  views:
    ...
    - view_id: empirical_mean_gain
      operator_ids: [empirical_mean_gain]
    - view_id: empirical_residual_gain
      operator_ids: [empirical_residual_gain]
    - view_id: empirical_full_gain
      operator_ids: [empirical_full_gain]
...
- view_id: example_gain_then_noise
  operator_ids: [tracewise_gain, colored_noise]
```
...
- view_id: invalid_gain_then_phase
  operator_ids: [tracewise_gain, phase_rotation_p10deg]
```
...
sampling:
  ...
  view_weights:
    global_gain: 0.2
    tracewise_gain: 0.2
    empirical_mean_gain: 0.2
    empirical_residual_gain: 0.2
    empirical_full_gain: 0.2
```
...
validation:
  ...
  seismic_views:
    parent_weights: {base: 0.5, variant: 0.5}
    view_weights:
      phase_minus: 0.07692307692307693
      phase_plus: 0.07692307692307693
      shift_minus: 0.07692307692307693
      shift_plus: 0.07692307692307693
      static_minus: 0.07692307692307693
      static_plus: 0.07692307692307693
      global_gain: 0.07692307692307693
      tracewise_gain: 0.07692307692307693
      empirical_mean_gain: 0.07692307692307693
      empirical_residual_gain: 0.07692307692307693
      empirical_full_gain: 0.07692307692307693
      white_noise: 0.07692307692307693
      colored_noise: 0.07692307692307693
```

该 checkpoint objective 明确定义为 **view-uniform objective**：variant 总权重中的 13 个视图逐 view 等权。这不是 family-uniform objective。三个家族另行报告等家族权重的诊断指标，但不参与 checkpoint 选择。
...
- [x] 将 science contract 升为 science-v4；
- [x] 建立共享公共配置 parser；
...
- [x] 保存 split assignment 内容哈希、view spec hash set 和 science-v4 identity。
...
| validation 与 checkpoint | `ginn_v2/composable.py`、`ginn_v2/checkpoint.py` | 深度 smoke 输出 base、14 view、3 组 family 和 paired 指标；`load_checkpoint` 严格读取 v5 provenance |
...
| amplitude | 0.5 | 0.5 | global、tracewise、axis-lateral、RGT-lateral、empirical-RGT 与 empirical-RGT-plus-random gain，组内等权 |
| noise | 0.5 | 0.5 | white、colored noise，组内等权 |
| operator | 0.5 | 0.5 | ±phase、±wavelet shift、±axis static，组内等权 |
| all | 0.5 | 0.5 | 全部 14 个配置视图，组内等权 |
...
- 未配置视图不被计算或物化；
- science-v4、projection、view、operator 和 random contract identity 可独立校验；
...
- Synthoseis-lite v5/science-v4 schema、canonical-only LFM、原子 operator grammar、规范化 view fingerprint 和命名随机流；
- 时间域与深度域共用的无视图分层 pilot 配置、二维粗尺度振幅场估计、分层聚合、振幅先验合同、原子发布、严格加载和视图解析；两个域只在真实剖面读取、层位轴与本域正演处使用各自 Adapter；
- 同一经验振幅先验驱动的均值、随机残差和完整模式三个成对视图；
...
上述命令已通过。随后使用当时的 operator-v1 深度配置执行了一个 `debug_attempt_limit=1`、仅保留 `none` 几何族的历史 smoke：输出目录发布成功，`run_summary.json` 为 `development_limited`，`realization_index.csv` 为 5 个父实现，`seismic_view_index.csv` 为 55 个视图行（5 × 11），manifest 含 contract fingerprint，HDF5 reader 成功读取 5 个父实现及其视图，失败 attempt 只出现在拒绝记录。该结果只记录共享 Pipeline 的历史验收，不是 operator-v2 的活动 benchmark。另一次 `calibrate` smoke 输出 `run_summary.json.status=success`，校准产物完整发布。最小测试 Adapter 也已从同一共享 Pipeline 生成完整 v5 双索引和 summary。
