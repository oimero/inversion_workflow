...
`realization_id` 唯一。该索引只包含完整发布、可被 consumer 使用的父实现，不保存失败 attempt，因此不含 `status`。拒绝原因进入 `attempts.csv` 和 `rejections.csv`。
...
- attempt 和拒绝原因写入 `attempts.csv` / `rejections.csv`；
- 其他 attempt 继续执行。
...
`split_assignment.csv` 至少记录 `realization_id`、`evaluation_role`、hash value、split 和 split contract identity。五组归因共用同一文件及其 SHA-256。
...
- [ ] 将 science contract 升为 science-v3，并删除旧 LFM/variant 版本字段；
- [ ] 建立共享公共配置 parser；
- [ ] 建立 `SyntheticBenchmarkPipeline`；
- [ ] 建立并接入时间域、深度域两个 `SyntheticDomainAdapter`；
- [ ] 统一 calibrate 发布；
- [ ] 统一 generate、scenario、attempt、evaluation role 和接受率编排；
...
```text
1A  v5 records、science contract、view spec、双索引和内存 Reader/Writer
1B  SeismicViewPipeline、operator registry、grammar 和随机身份
1C  共享 generate Pipeline 与测试 Adapter
1D  Depth Adapter 接入和深度域 smoke
1E  Time Adapter 接入和时间域 smoke
1F  共享 calibrate Pipeline
1G  删除旧路径、完成最终 artifact 校验并正式发布 v5
```
...
- [ ] 物化唯一的 `split_assignment.csv`，patch 构建只允许 strict split；
- [ ] patch catalog 改为每个父实现 patch 一行；
- [ ] 实现 `parent_balanced_seismic_view`；
- [ ] 将 view sampling 限定在各 synthetic loss block 内；
- [ ] 实现训练和 validation 的完整权重校验；
- [ ] 从 train 父实现 base 地震和 canonical background 生成共享 normalization；
- [ ] validation 使用固定显式权重；
- [ ] 输出 base、逐视图、分组和 paired 指标；
- [ ] weighted metric 接入 checkpoint selection 与 deployment eligibility；
- [ ] checkpoint 保存视图合同与实际采样计数；
- [ ] 拒绝 v4 benchmark 和旧 checkpoint 来源语义；
- [ ] 保存 split assignment、view spec hash set 和 science-v3 identity。
...
- split 只由带固定 contract 的 `split_assignment.csv` 所有；
- GINN 先选父实现再选视图；
...
- 共享 `SeismicViewPipeline`、`SyntheticBenchmarkPipeline` seam、时间/深度 domain Adapter、成功父实现/视图双索引和严格 v5 reader；
- 父实现事务提交、HDF5 视图路径、base-only normalization、GINN-owned `split_assignment.csv`、父实现均衡视图 sampler 和 weighted validation diagnostics；
...
被忽略的 `tests/` 已写入 v5 配置失败、算子随机身份、共享视图 Pipeline、父实现 sampler 不变性、split 合同和 GINN 验证权重测试，由用户在本机运行。正式五组深度域归因和活动视图人工裁决仍属于下一步实验工作，不在代码 smoke 中替代完成。
