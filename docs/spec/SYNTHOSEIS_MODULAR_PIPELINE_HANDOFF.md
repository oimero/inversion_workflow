...
- [ ] 为每个 synthetic source 物化 `split_assignment_<source_id>.csv`，patch 构建只允许 strict split；
...
- 共享 `SeismicViewPipeline`、`SyntheticBenchmarkPipeline` seam、时间/深度 domain Adapter、成功父实现/视图双索引和严格 v5 reader；当前 `SyntheticBenchmarkPipeline` 已接管视图与索引公共 seam，完整 calibrate/generate 生命周期仍按阶段 1C–1F 收口；
