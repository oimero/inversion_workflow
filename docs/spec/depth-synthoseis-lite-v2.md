...
`source_runs` 空值采用 `resolve_source_run()` 发现最新合格 run；非空值严格使用
显式目录。
...
不得使用 `(xline - xline_min)` 直接作为数组下标。当前回归约束固定为 inline
步长 1、xline 步长 4。
...
`maximum_allowed_vp_mps` 由冻结 AI–Vp 关系和校准允许的最大生成 AI 推导，必须有限、
为正。目标层段加完整 context 后若不能落在实际 survey TVDSS 轴内，section 失败，
不得缩短子波或减少 halo。
...
- `sample_id`、`sample_kind`、`source_sample_id`、`hdf5_group`；
...
3. 增加井顶—解释层面审计和 xline 步长 4 的 section 几何校验。
...
7. 升级 HDF5 writer、manifest、sample index 和 reader 到 v2。
8. 接通 GINN v2 最小 reader 契约；训练细节停止在本文边界。
...
8. Huber 背景与中心有限差分/已知异常点 fixture 一致。
9. NW8 只进入拥有完整上下界的 zone，其他可用 zone 不受影响。
10. 层位审计使用 inline/xline，不直接混用两套 XY；数值差异不触发门禁。

### 生成与正演

11. inline 步长 1、xline 步长 4 的折线重采样和层面采样正确。
12. 5 m/0.625 m 轴严格嵌套并吸附到 survey TVDSS 样点。
13. 带状 NumPy 深度正演与 dense `W_depth` 在多速度、非对称子波和边界样点逐点一致。
14. banded chunk 变化不改变结果；`return_operator=True` 仍返回完整矩阵。
15. highres/model 两套 Vp 均由各自 AI 派生，禁止 Vp 下采样替代。
16. observed、model-consistent、residual、LFM 和 mask 全为 N 点。
17. model-consistent 重算闭合；subgrid residual 等于两条地震逐点之差。
18. 上下文不足、非正 Vp、错轴、错单位和非有限输入明确失败。

### LFM、variant 与产物

19. 250 m/400 m 最小波长滤波响应符合空间频率解析预期。
20. 深度配置拒绝 Hz LFM 和 `twt/time` 纵向字段。
21. 子波相位/时间平移通过重新正演产生，不等同于直接旋转深度道。
22. ±2.5 m 静差不外推，并正确收缩 valid mask。
23. combined variant 执行顺序和参数记录固定。
24. 深度 HDF5 不含 highres seismic、canonical、probe 或无单位 sample axis。
25. reader 拒绝 v1、错 domain、错 axis、错 shape 和错 forward hash。
26. GINN v2 reader 使用 observed，physics target 使用 model-consistent，深度不丢首点。
27. time v2 fixture 保持现有 filtered/full 校准和 Robinson 正演行为。
