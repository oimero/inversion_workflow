...
时同时定义其最小高分 cell 数和允许 split，并升级 generator family 或 schema。
...
### 5.7 attempt、接受率与数据拆分

split 由 generate 冻结，训练端只消费，不得重新随机划分。原因是 split 属于 benchmark
身份，而不是某次模型训练的超参数；若交给训练端，同一父 realization 的相邻 patch、
LFM 或 seismic mismatch 变体很容易跨集合泄漏，不同模型也会得到不可比较的测试集。
...
现有 SHA-256 命名流和 `PCG64DXSM`；`assignment_hash` 使用独立、冻结的 split purpose，
不能依赖生成循环顺序或 Python `hash()`。manifest 冻结 seed 派生字段顺序、算法版本和
...
3. held-out family 的全部父 realization 进入 `test`；
4. 其他父 realization 按配置比例，在
   `section_id × geometry_family × geometry_direction × duration_mode` 层内以
   `assignment_hash` 稳定排序后分配；
5. 计数采用最大余数法；层内样本数足够时，每个非零比例 split 至少分得一个父样本；
6. base、其所有 mismatch variant 以及未来由它裁出的全部 patch 必须继承同一 split；
7. generate 过程中不得因拒绝率或 split 数量不足动态追加 attempt/seed。

`attempt_plan.csv` 是计划，不在生成后改写接受状态。实际 `ok/rejected`、原因和逐 split
接受数写入 `generation_qc.csv` 与 `scenario_catalog.csv`。每个 scenario 至少评估 20 次：
接受率低于 0.50 整次 generate 失败，位于 `[0.50,0.80)` 记 warning。最终 train、
validation、test 任一为空均失败；manifest 同时分别报告 test 中 seen 与 held-out family
的父样本数。

patch 裁取仍留给 GINN v2 训练专项设计，因为 patch 尺寸、stride、halo 和随机采样策略
属于模型消费方式。训练端可以按 epoch 随机裁 train patch，但只能在对应父 split 内；
validation/test patch 窗口必须在模型 run manifest 中冻结。benchmark 无需为了某个网络
预先复制 patch，也不能让 patch 级随机性反向修改父 split。
...
- `split`、`split_assignment_hash`、`held_out_geometry_family`；
...
- split 策略、比例、held-out family、逐 split/seen/held-out 计数及 `attempt_plan.csv` 哈希；
...
- 每个 variant 的 `parent_realization_id` 和 split 必须与所属 base 完全一致；
- `unassigned`、未知 split 或 held-out family 出现在非 test split 时立即失败。
...
训练入口不得生成新的 train/validation/test 划分。它只按 reader 暴露的父 split 过滤；
patch 无论在线还是离线构造都继承父 split。训练配置可以决定 patch 尺寸与采样密度，
不能改变 benchmark split 或把 held-out family 放回训练集。
...
这些指标读取冻结 benchmark split 和模型预测，不参与样本接受、训练分布、正演参数或
...
7. 在生成任何数组前冻结 attempt plan、父 realization split 和命名随机流契约。
...
19. 相同配置与 global seed 产生逐字节相同的 `attempt_plan.csv`，改变循环/图件/QC
    顺序不改变既有命名流。
20. held-out family 来自配置且只进入 test；代码中不存在固定 pinchout 分支。
21. 非 held-out 父 realization 按配置比例稳定分配，井/section/场景遍历顺序不影响结果。
22. base、mismatch variant 和训练端 patch 始终继承父 split；reader 拒绝跨 split 引用。
23. 不生成 `unassigned`；任一 split 为空或动态追加 attempt 都明确失败。
...
31. reader 拒绝 v1、错 domain、错 axis、错 shape 和错 forward hash。
...
- 所有可消费样本具有生成端冻结的父 realization split，且 held-out family 从未进入训练；
...
| patch 级拆分泄漏 | 相邻 patch 或同一 truth 的变体跨 train/test | generate 冻结父 split，训练 patch 只继承 |
