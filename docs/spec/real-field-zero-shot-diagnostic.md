...
- **R1 forward diagnostic**：对 R0 预测的 `log(AI)` 做固定正演诊断，拆解真实地震
  残差来源，为后续 sparse-well anchored adapter 判断依据。
...
稳定生产链仍终止于第五步。R0/R1 输出均为 research output，在完成未来 R2 adapter 验收前不得替代
前五步生产链。
...
- 模型候选必须能从 manifest 追溯到冻结 synthetic gate evidence 和 report-card SHA-256；不得把其他 seed、
...
并保留 `valid_mask_model=false` 作为输入通道和评价边界。当前分支不再依赖路径名拦截旧 LFM，
但必须通过 schema、字段和值域自检确认 LFM 是 `log(AI)` 语义；历史负对照必须显式标记为
`legacy_lfm_negative_control`。

若 LFM summary 或 `internal_horizon_continuity_qc.csv` 显示内部层位处存在 LFM 人工跃变，R1 必须标记
`lfm_internal_horizon_discontinuity`。该反射只能解释为 LFM 构造风险，不能解释为模型预测能力。
...
real_field_boundary:
  ...
  forward_diagnostic_erosion_s: <seconds or null>
  suggested_from_nominal_wavelet: true
```

实现可用第五步 nominal 子波 active half-support 生成建议值，但必须分别记录建议值和最终
使用值。若 `forward_diagnostic_erosion_s` 为 `null` 或未配置，R1 默认使用第五步 nominal
子波的 active half-support。样点数只由 `dt_s` 派生，并写入 manifest：
...
forward_diagnostic_erosion_samples = ceil(forward_diagnostic_erosion_s / dt_s)
```

`loss_or_eval_erosion_s`、`prediction_taper_halo_s` 和 `forward_diagnostic_erosion_s` 不得共用
同一个隐藏参数。R1 的侵蚀只作用于每条道每段连续有效 mask 的内侧统计区，不裁全局时间窗，
也不得改写 R0 预测。若 R0 需要对层外预测做 taper，必须只作用于 `pred_delta_vs_lfm`，
并在 summary 中记录。
...
- 子波来源和 SHA-256。
- 卷积 mode、子波中心和输出时间轴。
- `synthetic_twt_axis` 与对齐后的 `observed_twt_axis_after_alignment`。
- 因 N 到 N-1 正演轴转换而丢弃、valid-run erosion 或对齐的样点数。
- `time_alignment_mode`，必须与第五步和正演可观测性闸门的 forward core 一致。
- 是否使用 R0 mask erosion 与 R1 valid-run forward erosion。
...
- 进入 **future R2 sparse-well anchored adapter** 设计：当主要问题不能由输入准备、
  正演契约或合成覆盖不足解释，且留井/留簇验证设计清楚。
- 继续 synthetic 消融：当真实工区异常对应到合成基准未覆盖的场景，例如横向几何、LFM
  误差或失配类型不足。
- 暂缓真实工区适配：当 zero-shot 预测存在无法由 R1 解释的异常振幅、条带、锯齿或井旁
  不合理偏移。

R1 不能直接批准生产化 adapter。未来 R2 adapter 必须在留井/留簇验证设计清楚之后再进入。

R1 red flags 出现时，不得进入 future R2 adapter：
...
5. 边界参数以秒配置，样点数派生正确；loss/eval erosion、halo、forward erosion 不共用字段。
6. R0 预测不会被 R1 valid-run forward erosion 改写。
...
17. null-space energy red flag 能阻止进入 future R2 adapter。
