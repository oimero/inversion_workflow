...
- `D1（深度域工区）`：四模型 ablation 消融和 LFM 反事实分解；深度采样轴、模型和结论均独立于 T1。
...
| `20260628_real_delta_anchor_ph5_positive_signal` | `c7d751afe8db3d408afc304eede663e308aba9ab` | 完整冻结 PH5 held-out real-delta anchor 输出、checkpoint 与图件 | Positive proof-of-signal；继续 ablation real-delta 研究 |
...
- Trend / proportional LFM 四格反事实分解表明，proportional kriging 的最终收益主要来自 LFM 直接背景替换：`Y_TT → Y_PT` 已带来平均井相关系数 `+0.1350`，而网络条件输入效应 `Y_PT → Y_PP` 只再带来 `+0.0085`。这说明当前 ablation 更接近"给定 LFM 的地震残差修正器"，而不是独立恢复绝对低频阻抗的模型。该结果仍受同井 LFM 约束影响，不能替代 blind-well 或 R1 正演验证。

### 批次说明

| 批次 | 代码哈希 | 作用 | 当前状态 |
| --- | --- | --- | --- |
| `20260713_D1_ablation_lfm_audit` | `partial` | 冻结 D1 四模型 ablation 消融与 LFM 反事实分解 | 当前 D1 推荐入口 |
...
见 `note/summary/final_audit/20260713_D1_ablation_lfm_audit/`。
