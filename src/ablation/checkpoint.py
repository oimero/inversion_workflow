"""ablation canonical-increment checkpoint loading."""
...
from ablation.contracts import CHECKPOINT_SCHEMA_VERSION
...
def _validate_probability_contract(
    ...
    if not isinstance(value, dict):
        raise ValueError(f"ablation checkpoint {label} must be a mapping.")
    ...
    if not result or any(not math.isfinite(item) or item < 0.0 for item in result.values()):
        raise ValueError(f"ablation checkpoint {label} contains invalid weights.")
    if not math.isclose(sum(result.values()), 1.0, rel_tol=0.0, abs_tol=1e-12):
        raise ValueError(f"ablation checkpoint {label} must sum to one.")
    return result
...
def _validate_parent_view_sampling(value: object, *, label: str) -> None:
    ...
    if set(parent) != {"base", "variant"}:
        raise ValueError(f"ablation checkpoint {label} parent weights must contain base and variant.")
    raw_view = value.get("view_weights")
    if not isinstance(raw_view, dict) or "base" in raw_view:
        raise ValueError(f"ablation checkpoint {label} has invalid view weights.")
    ...
    if parent["variant"] > 0.0:
        if not view or any(not math.isfinite(item) or item <= 0.0 for item in view.values()):
            raise ValueError(f"ablation checkpoint {label} requires positive view weights.")
        if not math.isclose(sum(view.values()), 1.0, rel_tol=0.0, abs_tol=1e-12):
            raise ValueError(f"ablation checkpoint {label} view weights must sum to one.")
    elif view:
        raise ValueError(f"ablation checkpoint {label} must not contain views for a zero variant weight.")
...
def load_checkpoint(
    ...
    *,
    ...
    if str(checkpoint.get("schema_version") or "") != CHECKPOINT_SCHEMA_VERSION:
        raise ValueError(
            f"Unsupported ablation checkpoint schema in {path}; expected {CHECKPOINT_SCHEMA_VERSION}."
        )
    if str(checkpoint.get("output_semantics") or "") != "predicted_increment_log_ai":
        raise ValueError(
            f"ablation checkpoint in {path} does not use predicted_increment_log_ai semantics."
        )
    if list(checkpoint.get("input_channels") or []) != [
        "seismic", "input_lfm_log_ai", "valid_mask"
    ]:
        raise ValueError(
            f"ablation checkpoint in {path} has a non-canonical input channel contract."
        )
    ...
    if not isinstance(checkpoint.get("training_sources"), dict):
        raise ValueError(f"ablation checkpoint in {path} lacks training_sources provenance.")
    for key in (
        "benchmark_identity",
        ...
        if key not in checkpoint:
            raise ValueError(f"ablation checkpoint in {path} lacks {key} provenance.")
    if not isinstance(checkpoint.get("benchmark_identity"), dict):
        raise ValueError(f"ablation checkpoint in {path} has invalid benchmark identity provenance.")
    for source_id, identity in checkpoint["benchmark_identity"].items():
        if not isinstance(identity, dict):
            raise ValueError(
                f"ablation checkpoint in {path} has malformed benchmark identity for {source_id!r}."
            )
        required_identity = {
        ...
        if not required_identity.issubset(identity):
            raise ValueError(
                f"ablation checkpoint in {path} has incomplete benchmark identity for {source_id!r}."
            )
        if str(identity.get("schema_version")) != "synthoseis_lite_v5":
            raise ValueError(
                f"ablation checkpoint in {path} references a non-v5 benchmark source {source_id!r}."
            )
    sampling_contract = checkpoint.get("synthetic_sampling_contract")
    if not isinstance(sampling_contract, list):
        raise ValueError(f"ablation checkpoint in {path} has invalid synthetic sampling provenance.")
    for item in sampling_contract:
        if not isinstance(item, dict):
            raise ValueError(f"ablation checkpoint in {path} has a malformed synthetic sampling contract.")
        ...
        if not isinstance(item.get("validation"), dict):
            raise ValueError(f"ablation checkpoint in {path} lacks validation weight provenance.")
        validation_views = dict(item["validation"].get("seismic_views") or {})
        if not validation_views:
            raise ValueError(f"ablation checkpoint in {path} lacks validation seismic-view provenance.")
        _validate_parent_view_sampling(
            validation_views,
    ...
    if not isinstance(checkpoint.get("stage_lineage"), list):
        raise ValueError(f"ablation checkpoint in {path} lacks stage_lineage provenance.")
    split_contract = checkpoint.get("split_assignment_contract")
    if not isinstance(split_contract, dict):
        raise ValueError(f"ablation checkpoint in {path} lacks split assignment provenance.")
    ...
    if not required_split_fields.issubset(split_contract):
        raise ValueError(
            f"ablation checkpoint in {path} has incomplete split assignment provenance."
        )
    if str(split_contract.get("version")) != "parent_hash_split_v1":
        raise ValueError(f"ablation checkpoint in {path} uses an unsupported split contract.")
    if not isinstance(checkpoint.get("normalization_identity"), str) or not checkpoint["normalization_identity"]:
        raise ValueError(f"ablation checkpoint in {path} lacks normalization identity provenance.")
    sampling_statistics = checkpoint.get("sampling_statistics")
    if not isinstance(sampling_statistics, dict):
        raise ValueError(f"ablation checkpoint in {path} has invalid sampling statistics provenance.")
    for key in ("kind_counts", "view_counts", "parent_counts"):
        value = sampling_statistics.get(key)
        if value is not None and not isinstance(value, dict):
            raise ValueError(f"ablation checkpoint in {path} has invalid {key} statistics.")
    if sampling_contract and not isinstance(sampling_statistics.get("history"), list):
        raise ValueError(
            f"ablation checkpoint in {path} lacks per-epoch sampling history."
        )
    if "history" in sampling_statistics and not isinstance(
        sampling_statistics["history"], list
    ):
        raise ValueError(
            f"ablation checkpoint in {path} has invalid per-epoch sampling history."
        )
    for epoch_index, epoch_stats in enumerate(sampling_statistics.get("history", [])):
        if not isinstance(epoch_stats, dict) or not {
            ...
            raise ValueError(
                f"ablation checkpoint in {path} has incomplete sampling history at index {epoch_index}."
            )
    run_mode = str(checkpoint.get("run_mode") or "")
    if run_mode not in {"standard", "smoke"}:
        raise ValueError(
            f"ablation checkpoint in {path} lacks a valid run_mode (standard or smoke)."
        )
    development_limited = checkpoint.get("development_limited")
    if not isinstance(development_limited, bool):
        raise ValueError(f"ablation checkpoint in {path} lacks development_limited metadata.")
    deployment_eligible = checkpoint.get("deployment_eligible")
    if not isinstance(deployment_eligible, bool):
        raise ValueError(f"ablation checkpoint in {path} lacks deployment_eligible metadata.")
    stage_deployment_eligible = checkpoint.get("stage_deployment_eligible")
    if not isinstance(stage_deployment_eligible, bool):
        raise ValueError(
            f"ablation checkpoint in {path} lacks stage_deployment_eligible metadata."
        )
    if deployment_eligible != stage_deployment_eligible:
        raise ValueError(
            f"ablation checkpoint in {path} has inconsistent stage/deployment eligibility metadata."
        )
    if not isinstance(checkpoint.get("stage_deployment_eligibility_reason"), str):
        raise ValueError(
            f"ablation checkpoint in {path} lacks stage_deployment_eligibility_reason metadata."
        )
    if not isinstance(checkpoint.get("physics_closures"), list):
        raise ValueError(f"ablation checkpoint in {path} lacks physics_closures metadata.")
    stage_loss_blocks = checkpoint.get("stage_loss_blocks")
    if not isinstance(stage_loss_blocks, list):
        raise ValueError(f"ablation checkpoint in {path} lacks stage_loss_blocks metadata.")
    for item in stage_loss_blocks:
        if not isinstance(item, dict) or "weight" not in item or "update_interval" not in item:
            raise ValueError(
                f"ablation checkpoint in {path} has incomplete stage_loss_blocks metadata; "
    ...
    if not isinstance(checkpoint.get("stage_selection_metric"), str):
        raise ValueError(f"ablation checkpoint in {path} lacks stage_selection_metric metadata.")
    ...
    if "physics" in stage_kinds:
        ...
        if deployment_eligible != expected_eligible:
            raise ValueError(
                f"ablation checkpoint in {path} has invalid physics deployment eligibility; "
                "waveform-only and real-well physics checkpoints are diagnostic/experimental."
            )
    if run_mode == "smoke" and not development_limited:
        raise ValueError(
            f"ablation checkpoint in {path} marks a smoke run as not development_limited."
        )
    architecture = dict(checkpoint.get("architecture") or {})
    architecture_id = str(architecture.get("id") or "")
    if not architecture_id:
        raise ValueError("ablation checkpoint lacks the canonical architecture contract.")
    model, _ = build_model(
        architecture_id,
