...
核心公开对象
------------
1. WellControl: 井曲线控制描述。
2. LayerControlPoint: 层段比例坐标中的点级控制描述。
3. ZoneSliceModel: 单层段比例切片建模结果。
4. LayerConstrainedModelResult: 井曲线控制建模输出与覆盖统计。
5. LayerPointConstrainedModelResult: 点级控制建模输出与覆盖统计。
6. build_layer_constrained_model: 井曲线控制建模入口。
7. build_point_constrained_model: 点级控制建模入口。
"""
...
class LayerControlPoint:
    ...
    ``zone_name`` 接受 ``"top->bottom"`` 或 ``"top_bottom"`` 两种写法；
    ``u_in_zone`` 为层段内归一化比例坐标，0 表示顶界，1 表示底界。
    """
...
@dataclass
class LayerPointConstrainedModelResult:
    """点级控制的层位约束建模通用结果。"""
    ...
    samples: np.ndarray
    metadata: Dict[str, Any]
    point_controls: list[LayerControlPoint]
    zone_models: list[ZoneSliceModel]
    coverage_stats: Dict[str, Any]
...
def _point_zone_matches(point: LayerControlPoint, top_name: str, bottom_name: str) -> bool:
    zone = str(point.zone_name)
    return zone == _zone_key(top_name, bottom_name) or zone == f"{top_name}_{bottom_name}"


def _finite_point_control(point: LayerControlPoint) -> bool:
    values = [
        point.inline,
        point.xline,
        point.u_in_zone,
...
def _aggregate_duplicate_slice_points(
    ...
    for index, inline in enumerate(inlines):
        keys.setdefault((float(inline), float(xlines[index])), []).append(index)
...
def _build_zone_slice_model_from_points(
    ...
    *,
    ...
    finite_points = [point for point in point_controls if _finite_point_control(point)]
    for slice_idx, _u in enumerate(slice_u):
        ...
        control_inlines = np.asarray([point.inline for point in points], dtype=float)
        control_xlines = np.asarray([point.xline for point in points], dtype=float)
        control_values = np.asarray([point.value for point in points], dtype=float)
...
def build_point_constrained_model(
    target_layer: TargetZone,
    point_controls: list[LayerControlPoint],
    *,
    property_name: str,
    ...
    exact: bool = True,
    nugget: float = 0.0,
) -> LayerPointConstrainedModelResult:
    """基于层段比例坐标点控制构建三维采样体。
    ...
    target_layer : TargetZone
        目标层位对象。
    point_controls : list[LayerControlPoint]
        点级控制集合。每个控制点需给出层段名、层段内比例坐标、线号坐标与属性值。
    property_name : str
        当前建模属性名；所有有效控制点必须与它一致。
    boundary_extension_samples : int, default=0
        上下边界外延的样点数。
    n_slices : int, default=32
        每个层段的比例切片数量。
    ...
    LayerPointConstrainedModelResult
        建模输出、点控制副本与覆盖统计。
    """
    if not point_controls:
        raise ValueError("point_controls must contain at least one LayerControlPoint.")
    if n_slices < 2:
        raise ValueError(f"n_slices must be >= 2, got {n_slices}.")
    if boundary_extension_samples < 0:
        raise ValueError(f"boundary_extension_samples must be >= 0, got {boundary_extension_samples}.")

    requested_property = str(property_name)
    prepared_points = [
        LayerControlPoint(
            control_id=str(point.control_id),
            property_name=str(point.property_name),
        ...
        for point in point_controls
    ]
    finite_points = [point for point in prepared_points if _finite_point_control(point)]
    if not finite_points:
        raise ValueError("point_controls contain no finite positive-weight controls.")
    property_names = {point.property_name for point in finite_points}
    if property_names != {requested_property}:
        raise ValueError(
            f"Finite point controls must all use property_name={requested_property!r}, got {sorted(property_names)}."
        )
    ...
    coverage_stats: Dict[str, Any] = {
        ...
        "control_group_count": int(len({point.group_name for point in finite_points if point.group_name})),
        "groups": {},
        "zones": {},
    }
    for group_name in sorted({point.group_name for point in finite_points if point.group_name}):
        group_points = [point for point in finite_points if point.group_name == group_name]
        coverage_stats["groups"][group_name] = {
            "control_point_count": int(len(group_points)),
            "zone_names": sorted({str(point.zone_name) for point in group_points}),
        }

    base_zone_models: list[ZoneSliceModel] = []
    for top_name, bottom_name in target_layer.iter_zones():
        zone_points = [
            point for point in finite_points if _point_zone_matches(point, top_name, bottom_name)
        ]
        top_grid, bottom_grid = modeling_target_layer.get_zone_sample_index_grids((top_name, bottom_name))
    ...
    if extension_samples > 0:
        ...
        top_extension_model = ZoneSliceModel(
            ...
            top_grid=top_grid,
            bottom_grid=bottom_grid,
            slice_values=np.full((slice_u.size, n_il, n_xl), np.nan, dtype=float),
            slice_variance=np.full((slice_u.size, n_il, n_xl), np.nan, dtype=float),
    ...
    metadata = {
        ...
        "horizon_names": list(target_layer.horizon_names),
        "zone_names": [[zone.top_label, zone.bottom_label] for zone in modeled_zones],
        "control_group_names": sorted({point.group_name for point in finite_points if point.group_name}),
        "variance_volume_included": True,
    }

    return LayerPointConstrainedModelResult(
        volume=volume,
        variance_volume=variance_volume,
        geometry=dict(target_layer.geometry),
        ilines=ilines,
