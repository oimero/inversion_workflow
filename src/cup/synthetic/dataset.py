"""Consumer layer for frozen synthoseis-lite benchmark artifacts."""
...
class SynthoseisBenchmark:
    """Read-only accessor around ``synthetic_benchmark.h5`` and ``sample_index.csv``."""
    ...
    def sample_ids(self, *, kinds: set[str] | None = None, status: str = "ok") -> list[str]:
        frame = self.index
        if status:
