...
def _plot_background_and_residual(
    ...
    *,
    ...
    subset = samples[samples["well_name"].eq(well_name) & samples["zone_id"].eq(zone_id)].sort_values("twt_s")
    if subset.empty:
        return []
    generated: list[str] = []
    safe = f"{_sanitize(well_name)}_{_sanitize(zone_id)}"
    fig, ax = plt.subplots(figsize=(6.0, 4.0))
    ax.plot(subset["twt_s"], subset["filtered_log_ai"], "--", color="#4a5568", label="filtered LAS")
    ax.plot(subset["twt_s"], subset["background_log_ai"], color="#1a202c", label="background")
...
def write_calibration_figures(output_dir: Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for zone_id, zone_objects in objects.groupby("zone_id", sort=False):
        ...
        for ax, state in zip(axes, states):
            values = np.log(np.maximum(zone_objects.loc[zone_objects["state"].eq(state), "duration_s"], 1e-9))
            ...
            ax.grid(True, alpha=0.20)
        axes[-1].set_xlabel("log(duration_s)")
        fig.suptitle(f"Duration distribution: {zone_id}")
...
def _plot_section_geometry(output_dir: Path, generated: list[str], skipped: list[dict[str, Any]]) -> None:
    ...
    for section_id, section in frame.groupby("section_id", sort=False):
        ...
        for horizon, group in section.groupby("horizon_name", sort=False):
            ...
            ax.plot(group["lateral_m"], group["horizon_twt_s"], linewidth=1.5, label=str(horizon))
            filled = group[~group["trace_valid_control"].astype(bool)]
            if not filled.empty:
                ax.scatter(filled["lateral_m"], filled["horizon_twt_s"], s=10, marker="x", label=f"{horizon} filled")
        ax.invert_yaxis()
...
def _select_base_sample(index: pd.DataFrame, config: Mapping[str, Any]) -> pd.Series | None:
    ...
    if geometry_family:
        ...
        if not selected.empty:
            base = selected
    return base.sort_values(["section_id", "scenario_id", "attempt_id"]).iloc[0]
...
def _imshow_section(
    ...
    *,
    ...
    extent = [float(lateral[0]), float(lateral[-1]), float(twt[-1]), float(twt[0])]
...
def _plot_hdf5_examples(
    ...
    if not group_path:
        ...
        return
    safe = _sanitize(row["realization_id"])
    ...
    with h5py.File(h5_path, "r") as h5:
        ...
        twt_model = group["axes/twt_model_s"][()]
        twt_forward = group["axes/twt_forward_model_s"][()]
        generated.append(
            _imshow_section(
                group["truth/model_target_log_ai"][()],
                lateral=lateral,
                twt=twt_model,
                title=f"log(AI) target: {row['realization_id']}",
                cmap="viridis",
        ...
        generated.append(
            _imshow_section(
                ...
                lateral=lateral,
                twt=twt_forward,
                title=f"Model-consistent seismic: {row['realization_id']}",
                cmap="seismic",
                path=figures_dir / f"seismic_section_{safe}.png",
                symmetric=True,
            )
        )
        state = group["truth/state_id_highres"][()]
        twt_highres = group["axes/twt_highres_s"][()]
        ...
        ax.imshow(color_values[np.newaxis, :], aspect="auto", extent=[twt_highres[0], twt_highres[-1], 0, 1], cmap="coolwarm", vmin=0, vmax=2)
        valid_idx = np.where(color_values >= 0)[0]
        if len(valid_idx) > 0:
            ax.set_xlim(twt_highres[valid_idx[0]], twt_highres[valid_idx[-1]])
        ax.set_yticks([])
        ax.set_xlabel("TWT (s)")
        ax.set_title(f"State strip, central trace: {row['realization_id']}")
        generated.append(_finish(fig, figures_dir / f"state_strip_{safe}.png"))
        if "priors" in group and "residuals" in group:
            model = group["truth/model_target_log_ai"][()]
            lfm = group["priors/lfm_controlled_degraded"][()]
            residual = group["residuals/residual_vs_lfm_controlled_degraded"][()]
            ...
            for ax, values, title, cmap in [
                ...
                image = ax.imshow(values.T, aspect="auto", extent=[lateral[0], lateral[-1], twt_model[-1], twt_model[0]], cmap=cmap, vmin=vmin, vmax=vmax)
                ax.set_title(title)
                ax.set_ylabel("TWT (s)")
    ...
    if coeffs.empty:
        ...
        return
    subset = coeffs[coeffs["realization_id"].eq(row["realization_id"])]
    if subset.empty:
    ...
    ax.set_title(f"Actual c0(x): {row['realization_id']} / object {object_id}")
    ax.set_xlabel("lateral distance (m)")
    ax.set_ylabel("c0")
    ax.grid(True, alpha=0.25)
