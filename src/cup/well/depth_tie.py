...
def prepare_vertical_depth_extraction(
    ...
    *,
    ...
    continuous_logs = _crop_logset(logset_md, start, stop)
    log_tvdss = np.asarray(continuous_logs.basis, dtype=np.float64) - kb
    ...
    relative_rows = grid.build_local_tdt_from_vp(
        tvdss_m=log_tvdss,
        vp_mps=np.asarray(continuous_logs.Vp.values, dtype=np.float64),
        md_m=np.asarray(continuous_logs.basis, dtype=np.float64),
        origin_twt_s=0.0,
    ...
    report = {
        ...
        "continuous_log_md_start_m": float(continuous_logs.basis[0]),
        "continuous_log_md_end_m": float(continuous_logs.basis[-1]),
        "continuous_log_sample_count": int(continuous_logs.basis.size),
    }
    return DepthExtractionInputs(
        logset_md=continuous_logs,
