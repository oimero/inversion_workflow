...
class EnhanceV2Config:
    """Frozen real-field configuration for the stage-2 enhancement module.

    The public training interface consumes this one object.  Paths are resolved
    by :meth:`from_yaml`; numerical scales and training choices remain in the
    experiment YAML instead of being embedded in the implementation.
    """
    ...
    training_pairs: int
    fixed_monitor_pairs: int
    batch_size: int
    patch_radius: int
    formal_epochs: int
    maximum_adjustment_runs: int
    adjustment_epochs: int
    residual_patch_min_m: float
    residual_patch_max_m: float
    residual_scale_min: float
    residual_scale_max: float
    zero_injection_fraction: float
    nonzero_target_weight: float
    input_channels: int
    hidden_channels: int
    num_res_blocks: int
    dilations: tuple[int, ...]
    ...
    grad_clip: float
    lambda_direct: float
    lambda_highpass: float
    lambda_rms: float
    adjustment_rms_weight_multiplier: float
    highpass_window_samples: int
    rms_window_samples: int
    well_anchor_weight: float
    well_anchor_normalize: bool
    derivative_normalization: float | None
    min_prediction_rms_ratio: float
    max_prediction_rms_ratio: float
    min_wells_improved: int
    seed: int
    device: str
    review_section_traces: int
    log_interval: int
    ...
    def from_yaml(
        ...
        *,
        ...
        missing = [key for key in required_inputs if not str(inputs.get(key) or "").strip()]
        if missing:
            raise ValueError(f"Enhance V2 config is missing explicit inputs: {missing}")
        ...
        kwargs: dict[str, Any] = {
            "workflow_config": resolve(inputs["workflow_config"]),
            "well_control_run_dir": resolve(inputs["well_control_run_dir"]),
            "lfm_run_dir": resolve(inputs["lfm_run_dir"]),
            "lfm_variant_id": str(inputs["lfm_variant_id"]),
            "ginn_checkpoint": resolve(inputs["ginn_checkpoint"]),
            "ginn_config": resolve(inputs["ginn_config"]),
            "forward_model_inputs": resolve(inputs["forward_model_inputs"]),
            "result_dir": (result_dir or resolve(str(raw.get("result_dir") or "experiments/enhance_v2/results"))),
            "trusted_well_names": tuple(str(item) for item in training.pop("trusted_well_names", ())),
        }
        kwargs.update(training)
        kwargs.update(selection)
        if "dilations" in kwargs:
            kwargs["dilations"] = tuple(int(value) for value in kwargs["dilations"])
        value = cls(**kwargs)
        value.validate()
        return value
    ...
    def validate(self) -> None:
        paths = (
            ...
            self.well_control_run_dir,
            self.lfm_run_dir,
            self.ginn_checkpoint,
            self.ginn_config,
            self.forward_model_inputs,
        )
        missing = [str(path) for path in paths if not path.exists()]
        if missing:
            raise FileNotFoundError("Enhance V2 input does not exist: " + ", ".join(missing))
        if not self.lfm_variant_id or not self.trusted_well_names:
            raise ValueError("lfm_variant_id and trusted_well_names must be explicit and non-empty.")
        if len(set(name.casefold() for name in self.trusted_well_names)) != len(self.trusted_well_names):
            raise ValueError("trusted_well_names must be unique.")
        for name in (
            "body_smoothing_fwhm_m",
            "residual_patch_min_m",
            "residual_patch_max_m",
            "residual_scale_min",
            "residual_scale_max",
            "learning_rate",
            "weight_decay",
            ...
            "lambda_highpass",
            "lambda_rms",
            "min_prediction_rms_ratio",
            "max_prediction_rms_ratio",
        ):
            value = float(getattr(self, name))
            if not np.isfinite(value) or value < 0.0:
                raise ValueError(f"{name} must be finite and non-negative.")
        if self.body_smoothing_fwhm_m <= 0.0 or self.residual_patch_min_m <= 0.0:
            raise ValueError("body_smoothing_fwhm_m and residual_patch_min_m must be positive.")
        if self.residual_patch_max_m < self.residual_patch_min_m:
            raise ValueError("residual_patch_max_m must not be smaller than residual_patch_min_m.")
        if self.residual_scale_max < self.residual_scale_min or self.residual_scale_min < 0.0:
            raise ValueError("residual_scale range is invalid.")
        if not np.isfinite(self.adjustment_rms_weight_multiplier) or self.adjustment_rms_weight_multiplier <= 1.0:
            raise ValueError("adjustment_rms_weight_multiplier must be greater than one.")
        if not 0.0 <= self.zero_injection_fraction <= 1.0:
            raise ValueError("zero_injection_fraction must be within [0, 1].")
        if not np.isfinite(self.nonzero_target_weight) or self.nonzero_target_weight < 1.0:
            raise ValueError("nonzero_target_weight must be finite and at least one.")
        if self.training_pairs <= 0 or self.fixed_monitor_pairs <= 0 or self.batch_size <= 0:
            raise ValueError("training_pairs, fixed_monitor_pairs, and batch_size must be positive.")
        if self.formal_epochs <= 0 or self.maximum_adjustment_runs < 0 or self.adjustment_epochs <= 0:
            raise ValueError("epoch counts are invalid.")
        if self.input_channels != 6 or self.hidden_channels <= 0 or self.num_res_blocks <= 0:
            raise ValueError("Enhance V2 expects six input channels and positive network dimensions.")
        if len(self.dilations) != self.num_res_blocks or any(int(value) <= 0 for value in self.dilations):
            raise ValueError("dilations must match num_res_blocks and be positive.")
        if self.kernel_size <= 0 or self.kernel_size % 2 == 0:
            raise ValueError("kernel_size must be a positive odd integer.")
        if self.grad_clip < 0.0 or self.highpass_window_samples <= 0 or self.rms_window_samples <= 0:
            raise ValueError("optimizer and filter settings are invalid.")
        if self.min_wells_improved <= 0 or self.min_wells_improved > len(self.trusted_well_names):
            raise ValueError("min_wells_improved must be within the trusted-well count.")
        if self.seed < 0 or self.patch_radius <= 0 or self.review_section_traces <= 0 or self.log_interval <= 0:
            raise ValueError("seed, patch_radius, review_section_traces, and log_interval are invalid.")
