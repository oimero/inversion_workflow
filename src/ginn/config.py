...
class GINNConfig:
    ...
    lfm_precomputed_file: Path | None = Path("your_lfm_precomputed_file")  # lfm_source=lfm_precomputed_file 时的 LFM 文件。
    lfm_initial_inversion_file: Path | None = Path("your_lfm_initial_inversion_file")  # lfm_source=lfm_initial_inversion_file 时的初始反演体。
    ...
    wavelet_file: Path | None = Path("your_precomputed_wavelet.csv")  # wavelet_source=precomputed_wavelet 时的子波 CSV。
    ...
    # ── 优化与训练循环 ────────────────────────────────────────
    # 常规 Adam 训练参数。若未来加入预训练或混合训练，请优先在配置里显式增加
    # 对应学习率/epoch 参数，不要把新的训练常数藏在 trainer 里。
