...
class DepthGINNConfig:
    ...
    # ── 低频模型 ──────────────────────────────────────────────
    # LFM 是 AI 的低频锚点，网络只预测相对 LFM 的残差。
    ...
    lambda_tv: float = 0.1  # 残差 TV 正则化权重，抑制层内高频 ringing。
    ...
    log_ai_anchor_radius_xy_m: float = 256.0  # anchor 影响半径（XY 米制）；0=仅中心道。
    ...
    checkpoint_dir: Path = Path("scripts/output/ginn_train_depth/checkpoints")  # checkpoint 输出目录。
    ...
    def from_dict(cls, data: Dict[str, Any], *, base_dir: Path | None = None) -> "DepthGINNConfig":
        ...
        optional_path_fields = {"wavelet_file", "dynamic_gain_model", "log_ai_anchor_file"}
        for field_name in _PATH_FIELDS:
