...
from wtie.modeling.reflectivity import (
...
from wtie.modeling.wavelet import RandomButterworthTools, RandomOrmbyTools, RandomRickerTools, RandomWaveletCallable
from wtie.processing.taper import CosinePowerTaper
from wtie.utils.types_ import _path_t


def create_or_load_synthetic_dataset(
    h5_file: _path_t, h5_group_training: str, h5_group_validation: str, parameters: dict
):
    ...
    rand_rick_func = RandomRickerTools(f_range=(fc_min, fc_max), dt=dt, n_samples=wavelet_size)
    ...
    rand_butt_func = RandomButterworthTools(
        lowcut_range=(low_min, low_max),
    ...
    rand_om_func = RandomOrmbyTools(
        (f0_min, f0_max), (f1_min, f1_max), (f2_min, f2_max), (f3_min, f3_max), dt=dt, n_samples=wavelet_size
    )
    ...
    perts += [
        perturbations.RandomSimplexNoise(
            scale_range=(0.002, 0.01), variation_scale_range=(wavelet_size / 45, wavelet_size / 25)
        )
    ]
    perts += [perturbations.RandomConstantPhaseRotation(angle_range=(-55, 55))]
    # perts+=[perturbations.RandomTimeShift(max_samples=4)]
    perts += [perturbations.RandomSimplextPhaseRotation(scale_percentage_factor=3.0, max_abs_angle=30)]
    ...
    perts = perturbations.Compose(perts, random_switch=False, p=0.05)
    ...
    # uniform
    rnd_u_chooser = RandomUniformReflectivityChooser(reflectivity_size, sparsity_rate_range=(sr_min, sr_max))

    # weak uniform
    rnd_weak_u_chooser = RandomWeakUniformReflectivityChooser(
        reflectivity_size, sparsity_rate_range=(sr_min, sr_max), max_amplitude_range=(max_min, max_max)
    )

    # simplex
    vs_min = params["reflectivity"]["simplex"]["variation_scale_min"]
    vs_max = params["reflectivity"]["simplex"]["variation_scale_max"]
    rnd_s_chooser = RandomSimplexReflectivityChooser(
        reflectivity_size, sparsity_rate_range=(sr_min, sr_max), variation_scale_range=(vs_min, vs_max)
    )
    ...
    # -------------
    params_modeler = None
    modeler = ConvModeler(kwargs=params_modeler)  # type: ignore

    # -------------
    # Creators
    # -------------
    training_creator = SyntheticDataCreator(
    ...
    validation_creator = SyntheticDataCreator(
