...
from wtie.learning.model import VariationalEvaluator, VariationalNetwork
from wtie.modeling.modeling import ConvModeler
from wtie.processing import grid
from wtie.processing.logs import interpolate_nans, smoothly_interpolate_nans
...
def load_wavelet_extractor(training_parameters: dict, networ_state_dict: Path) -> VariationalEvaluator:
    ...
    evaluator = VariationalEvaluator(
        network=net, expected_sampling=expected_sampling_rate, state_dict=networ_state_dict
    )
...
def load_poseidon_data(folder: Path, well: str = "torosa1") -> InputSet:
    ...
    return InputSet(logset_md, seismic, table, wellpath)
...
def _import_torosa1_well_path(folder: str) -> grid.WellPath:
    ...
    _wp = pd.read_csv(
        file_path, header=1, delimiter=r"\s+", names=("Depth", "Inclination", "Azimuth"), usecols=range(1, 4)
    )
    # datum
    kb = 22.9  # meters
...
def _import_boreas1_logs(folder: str) -> grid.LogSet:
    ...
    rho2 = smoothly_interpolate_nans(
        _rho, despike_params=dict(median_size=41, threshold=1.0), smooth_params=dict(std=4.0)
    )
...
def _import_boreas1_well_path(folder: str) -> grid.WellPath:
    ...
    _tvd = grid.WellPath.get_tvdkb_from_inclination(_wp.loc[:, "Depth"].values, _wp.loc[:, "Dev"].values[:-1])
    _tvd = grid.WellPath.tvdkb_to_tvdss(_tvd, kb)
...
def _import_volve_seismic(folder: str, prestack: bool = False) -> grid.seismic_t:
...
def _import_volve_well_path(folder: str) -> grid.WellPath:
    ...
    _tvd = grid.WellPath.get_tvdkb_from_inclination(
        _wp.loc[:, "MD (kb) [m]"].values, _wp.loc[:, "Inclination"].values[:-1]
    )
    _tvd = grid.WellPath.tvdkb_to_tvdss(_tvd, kb)
