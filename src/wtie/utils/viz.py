...
def plot_wavelet(
    ...
    axes[0].fill_between(
        wavelet.basis, wavelet.values, where=(wavelet.values >= 0.0), color="b", alpha=0.8, interpolate=True
    )
    axes[0].fill_between(
        wavelet.basis, wavelet.values, where=(wavelet.values < 0.0), color="r", alpha=0.8, interpolate=True
    )
...
def plot_prestack_wavelet(
    ...
    for i, ax in enumerate([axes[0], axes[1], axes[2]]):
        ...
        ax.fill_between(wavelet.basis, _values, where=(_values >= 0.0), color="b", alpha=0.8, interpolate=True)
        ax.fill_between(wavelet.basis, _values, where=(_values < 0.0), color="r", alpha=0.8, interpolate=True)
    ...
    for i, idx in enumerate(range(3, 6, 1)):
        ...
        if wavelet[theta].uncertainties is not None:
            ...
            # phase
            axes[idx + 3].plot(ff, wavelet[theta].uncertainties.phase_mean, color="gray", alpha=0.9, lw=0.9)
            axes[idx + 3].fill_between(
                ff,
                wavelet[theta].uncertainties.phase_mean - wavelet[theta].uncertainties.phase_std,
                wavelet[theta].uncertainties.phase_mean + wavelet[theta].uncertainties.phase_std,
                color="gray",
                alpha=0.6,
            )
...
def plot_wellpath(wellpath: grid.WellPath, figsize: Tuple[int, int] = (5, 5), fig_axes: tuple = None) -> plt.subplots:
    if fig_axes is None:
        fig, ax = plt.subplots(figsize=figsize)
    else:
        fig, ax = fig_axes
...
def plot_td_table(
    table: grid.TimeDepthTable, figsize: Tuple[int, int] = (4, 4), plot_params: dict = None, fig_axes: tuple = None
) -> plt.subplots:
    if fig_axes is None:
        fig, ax = plt.subplots(figsize=figsize)
    else:
...
def plot_reflectivity(reflectivity: grid.Reflectivity, figsize: tuple = (3, 5), fig_axes: tuple = None) -> plt.subplots:
    x_picks = np.where(reflectivity.values != 0)[0]
...
def plot_trace(
    trace: grid.BaseTrace, figsize: Tuple[int, int] = (3, 5), fig_axes: tuple = None, plot_params: dict = None
) -> plt.subplots:
    if fig_axes is None:
        fig, ax = plt.subplots(figsize=figsize)
    else:
...
def plot_wiggle_trace(
    ...
    for offset in np.linspace(0.0, scaling * 1.0, num=repeat_n_times):
        ...
        ax.fill_betweenx(trace.basis, offset, _x, where=(_x >= offset), color="b", alpha=0.6, interpolate=True)
        ax.fill_betweenx(trace.basis, offset, _x, where=(_x < offset), color="r", alpha=0.6, interpolate=True)
...
def plot_trace_as_pixels(
    ...
    im = ax.imshow(pixels.T, interpolation="bilinear", aspect="auto", extent=extent, **im_params)
...
def plot_prestack_trace_as_pixels(
    ...
    im = ax.imshow(pixels.T, extent=extent, interpolation="bilinear", aspect="auto", **im_params)
...
def plot_dynamic_xcorr(
    dxcorr: grid.DynamicXCorr, figsize: Tuple[int, int] = (4, 5), fig_axes: tuple = None, im_params: dict = None
) -> plt.subplots:
    if fig_axes is None:
        fig, ax = plt.subplots(figsize=figsize)
    else:
    ...
    im = ax.imshow(dxcorr.values, extent=extent, interpolation="bilinear", aspect="auto", **im_params)
...
def plot_warping(
    ...
    for r_c, c_c in path:
        ...
        con = ConnectionPatch(
            xyA=[r_c_value, s1[r_c]],
            coordsA=ax.transData,
            xyB=[c_c_value, s2[c_c]],
